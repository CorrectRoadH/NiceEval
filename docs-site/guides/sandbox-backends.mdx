...
The sandbox backend is the infrastructure that creates and manages isolated runtime environments. [NiceEval](https://niceeval.com/) wraps all backends behind the same `Sandbox` interface, so adapters do not need to know whether the current backend is local Docker, a Vercel micro-VM, or a third-party provider.
...
Backend selection only lives in code. There is no CLI flag, and [NiceEval](https://niceeval.com/) does not auto-detect a backend. Set the `sandbox` field in the experiment, or set it in `niceeval.config.ts` as a project-level fallback:
...
If neither the experiment nor `niceeval.config.ts` sets `sandbox`, [NiceEval](https://niceeval.com/) fails when it tries to create the environment instead of guessing one.

The SDKs for the three built-in backends do not install alongside NiceEval. Install whichever one you use, so your project does not carry dependencies (and their native build scripts) it does not need:
