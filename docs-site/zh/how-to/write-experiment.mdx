...
| `sandbox` | sandbox agent 使用的固定 spec，或按 eval 环境 profile 返回 spec 的 resolver；spec 上可以链 `.setup()` / `.teardown()` 挂环境钩子 |
...
一批真实任务可能需要不同版本的运行时和依赖。eval 用 `environment` 声明一个与 provider 无关的 profile ID；experiment 再把它映射到具体模板或快照：
...
export default defineExperiment({
  agent: codexAgent(),
  sandbox: ({ eval }) => {
    if (eval.environment === "python-3.9-astropy-4.2") {
      return e2bSandbox({ template: "codex-python39" });
    }
    return e2bSandbox({ template: "codex-default" });
  },
});
```

`environment` 是非空的稳定字符串，不是包版本约束。resolver 必须为每个分支返回一个具体 Sandbox spec。NiceEval 在启动任何 Sandbox 前解析所有选中的 eval；返回空值或抛错会让本次运行直接报配置错误。remote Agent 不创建 Sandbox，因此不会调用 resolver。
