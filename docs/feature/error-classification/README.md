...
turn 失败没有 provisioning 那类「远端可能已创建计费实例」需要对账的歧义后果,只需要一个轴:是否瞬时。形状对齐 [`SandboxIoErrorKind`](../sandbox/architecture.md#已创建-sandbox-的文件-io-重试)而不是 `SandboxProvisionErrorKind`。
...
分类精度按 adapter 分别实现:不同 CLI / SDK 的错误形状不同,adapter 可自带分类器覆盖兜底(可选能力,挂在 [agent 契约](../adapters/architecture/agent-contract.md)上,与 sandbox 各 provider 自带 `classifyProvisionError` 同一套路);外层有一个保守的兜底分类器,复用 sandbox IO 分类器的正则形状——限流关键字 → `rate_limit`,连接建立层错误 → `network`,其余 → `unknown`。
...
执行体形状对齐 [sandbox 重试](../sandbox/architecture.md#provisioning-失败与重试):

- 封顶 4 次尝试(首次 + 至多 3 次重试)。
- 指数退避 + 全抖动,基数 5 秒。基数比 provisioning 的 1 秒大一个量级:限流窗口通常以十秒计,过小的基数只会让前几次重试在同一个限流窗口里白烧尝试次数。
- 退避期间释放全局并发槽位(与 `ProvisionSlot` 同一接口),睡醒后重新排队——被限流的一批 attempt 不占着并发名额陪睡。
- 重试进度走 activity 反馈,不产生 diagnostic:这是正常自愈过程,不是需要留痕的 warning。
- 重试不延长任何预算:attempt 外层 deadline 原样生效,退避睡眠可被 interruption 干净打断,不新增第二套超时语义。

重试封顶后,`agent.send()` 最终返回的失败 Turn 照旧走 `expectOk()` → `TurnFailed` → `AttemptError{code: "turn-failed"}` 路径,下游契约不变。
...
- [Adapter · agent 契约](../adapters/architecture/agent-contract.md) —— adapter 可选分类器的挂载面。
