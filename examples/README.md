...
| [`zh/eval/openllmetry/`](zh/eval/openllmetry/) · [`zh/eval/openinference/`](zh/eval/openinference/) · [`zh/eval/custom-genai/`](zh/eval/custom-genai/) | 走 OTel 通道接入对应 origin 应用 |
...
| [`zh/origin/openllmetry/`](zh/origin/openllmetry/) | 工具调用聊天服务 + **OpenLLMetry** 埋点（`@traceloop/node-server-sdk` 不认标准 OTLP 端点环境变量，需代码转译） | `node:http` + JSON（o11y 演示，无 SDK 规定的前端协议） |
| [`zh/origin/openinference/`](zh/origin/openinference/) | Python / FastAPI + LangChain agent + **OpenInference / Phoenix** 埋点 | FastAPI + JSON（同上） |
| [`zh/origin/custom-genai/`](zh/origin/custom-genai/) | 不用 vendor SDK，`@opentelemetry/api` **手写 GenAI 语义约定 span** | `node:http` + JSON(同上) |

其中 `origin/langgraph`、`origin/openllmetry`、`origin/openinference`、`origin/custom-genai` 同时是[连接可观测性指南](../docs-site/zh/guides/connect-otel.mdx)「2. 应用侧」各 tab 的完整可跑版本；`origin/claude-agent-sdk`、`origin/codex-sdk`、`origin/vm0` 对应仓库根 README「Agent Frameworks」Roadmap 的条目（Roadmap 勾选追踪 adapter 实现进度，不是示例有无）。
