...
description: "用 defineAgent 包装任意函数或 HTTP 服务为 niceeval agent，把响应映射到标准事件流，并按名字注册。"
---

`defineAgent` 用于连接不需要 sandbox 的被测对象：进程内函数、本地服务、远程 HTTP endpoint、WebSocket agent 或任何自定义框架。

不确定该做到哪一档（事件流、多轮、HITL、tracing）？先读[渐进式接入你的 agent](/zh/guides/connect-your-agent)——从 10 行最小接入开始逐档往上，每档解锁一组具体断言。
