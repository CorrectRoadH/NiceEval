...
See [coding-agent-skill](https://github.com/CorrectRoadH/coding-agent-skill) for a runnable reference.
...
You can also use `t.sandbox.writeFiles()` to add small seed files inside a single eval, which is a good fit for small tasks or security probes.
...
`t.sandbox.fileChanged()` is a scoped assertion: it checks the sandbox diff after the run to confirm the target file actually changed.
