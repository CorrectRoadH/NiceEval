...
<ParamField body="t.check(value, assertion)" type="(value: unknown, assertion: Assertion) => AssertionHandle">
  记录值断言，失败不会立即中断 test。
</ParamField>

<ParamField body="t.require(value, assertion)" type="(value: unknown, assertion: Assertion) => Promise<unknown>">
  前置条件断言（异步，需 `await`），失败会立即中断 test。
</ParamField>
