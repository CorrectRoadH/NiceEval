...
- 不同 prompts 或 feature flags（要求 Tier 2 接入：变体在应用内部，需要应用把它暴露成 experiment 可选的配置，经 `flags` → `ctx.flags` 透传）。
- 不同 sandbox 后端。
- 同一任务的 pass@N。

Tier 1 / Tier 2 的定义见[接入分两个 Tier](/zh/guides/connect-your-agent#接入分两个-tier)。
