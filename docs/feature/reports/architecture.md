...
分区发生在 Scope 已完成范围收窄和现刻水位选择之后、任何指标计算之前。每个组独立调用 `scopeSummaryData`、`metricScatterData` 与 `experimentListData`；所以组外 attempt 不可能污染该组的坐标尺度、series 连线、成功率、成本、排序或缺数据计数。Scope warning 仍属于整份选择，由页首的 `ScopeWarnings` 统一显示，不复制进每组。
...
裸 `show` 与裸 `view` 不是第二条路径：宿主默认装载的就是 `niceeval/report/built-in` 的默认导出——报告、Attempts、Traces 三页的普通 `defineReport`（全文见 [Library · 内建报告](library/built-in.md)），与任何 `--report` 文件走同一条 `装载 → resolve → validate → render` 管线。「builtin」不是装载逻辑里的类别，只是宿主默认拿哪个值的事实；标题取值链与装载规则因此对默认视图和自定义报告一致生效。字段穷尽见 [Library · 外壳与多页](library/shell.md)。
...
报告定义拥有全部页面内容——包括裸宿主导航里的 Attempts、Traces：它们是[内建报告](library/built-in.md)声明的普通页（`AttemptList` / `TraceWaterfall` 组件），换 `--report` 后要不要它们由报告文件决定。宿主没有保留内容，保留的是机器加一个恒定品牌位，清单穷尽如下：

- **管线与路由**：装载 → resolve → validate → render、`#/page/<id>` / `--page` 页寻址、导航条的渲染（渲染什么完全由页列表与外壳声明决定）、语言切换。
- **attempt 详情路由**：`view` 的 `#/attempt/@<locator>` 与 `show @<locator>`（及其证据切面）。它不是页、不占导航。本地宿主与 `show` **对完整结果根解析**它——不随报告槽的 Scope 收窄，手输或旧书签的深链也能打开；导出站只携带收窄后产物内的证据，页面 Scope 能引用的 attempt 恒在产物内，范围外的 locator 如实显示缺失。「报告里每个数字能下钻到证据」这条不变量在所有宿主都由它保证，不再依赖任何恒在的证据页。
...
「深链恒在」与「页共享收窄后的 Scope」因此不再冲突：Attempts 页作为普通页跟随收窄（页是对同一批数据的不同看法），本地被收窄滤掉的 attempt 仍可经详情路由打开。Traces 的 text 面也不再是特例——`TraceWaterfall` 的 text 面是带 `--timing` 下钻命令的 attempt 索引（[契约](library/site-components.md#tracewaterfall)），符合「索引终结于可执行命令」的省略规则。

### text 面的省略规则

两面同源不等于两面同长。数据组件在两个面输出同一份终值；web 的浏览增强（tab 切换、排序、过滤）在 text 面没有交互，但其覆盖的内容全量可读；纯视觉与纯 web 操作件（`Style`、`className`、`PoweredBy` 品牌行、`CopyFixPrompt` 按钮）与外壳的 web 字段（`links`、`footer`、`scripts`、`styles`）在 text 面零输出。text 面折叠成索引的只有带可复制下钻命令的结构——可比组、页与 `TraceWaterfall` 的 attempt 行；tab 没有选择器，索引只能是死路，所以既不索引也不省略。
...
| 默认填充 | [内建报告](library/built-in.md)首页（报告页）：`ExperimentComparison` 多组时只输出组索引与单组查看命令，单组时输出该组独立的成本 × 端到端成功率散点与 `ExperimentList`；尾部附 Attempts / Traces 页索引 | 同一内建报告：初始页是报告页，导航含三页；`ExperimentComparison` 完整组索引 + 当前组独立的散点与可排序、可过滤 `ExperimentList` 固定列表格；切组不重新读取 Scope |
| attempt 下钻 | `niceeval show @<locator>` | `#/attempt/@<locator>` |
| 证据 | `--source` / `--execution` / `--timing` / `--diff` | Attempt 详情路由（modal） |
| 自定义 | `--report <file>`（单页或多页定义） | `--report <file>`（单页或多页定义） |
| 页选择 | `--page <id>`；渲染初始页，多页时尾部附其余页索引 | `#/page/<id>` 路由；`--page <id>` 定初始页 |

裸 `show` 与裸 `view` 只是在同一默认 definition 上选择不同渲染面；显式 `--report` 也替换同一个报告槽。`view` 的导航条与 attempt 详情路由是机器不是内容（[边界清单](#宿主保留的只有机器)）；attempt locator 仍由宿主注入，组件中的证据引用继续通向 attempt 详情。
...
`view --out` 把报告 HTML、attempt 详情壳和前端会读取的 artifact 一起导出。报告 HTML 不是结果格式，`__NICEEVAL_VIEW_DATA__` 也不是编程读取契约；程序消费结果应使用 `niceeval/results`。
