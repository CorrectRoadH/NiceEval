...
//
// pi 的模型只能在进程启动时通过 AGENT_MODEL 读一次(agent.ts 里 `process.env.AGENT_MODEL ?? "deepseek-v4-flash"`
// 是模块加载时求值的,不支持按请求切换),所以模型对比不能共用一个 server:按 model 分桶,
// 每个 model 各自的实例各拉各的进程、各挑各的空闲端口,互不干扰、可并行跑。
 ...
 * 复用同一个实例(session 存在 server 内存里,中途重启会丢会话,见 agent.ts 头注释)。
 */
export async function ensureServer(options: EnsureServerOptions = {}): Promise<ServerHandle> {
  ...
  const ready: Promise<ServerHandle> = (async () => {
    ...
    throw new Error(
      `pi-sdk server(model=${key})在 30s 内未就绪:${baseUrl}/healthz。子进程 stderr:\n${stderr}`,
    );
  })();
