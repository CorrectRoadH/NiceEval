...
import { codexAgent } from "fasteval";
...
fasteval does not provide framework-level lifecycle hooks. Upload starter files and run setup commands inside `test(t)`, or put adapter-specific preparation in the agent adapter's `setup`.
