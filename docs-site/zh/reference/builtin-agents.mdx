...
- **T2 会话**：`messages` 历史由工厂管理，`isNew` 开新会话线、同 id 续接。
...
- 被测对象是 AI SDK 应用（进程内函数 / 工具循环）：直接用 `aiSdkAgent`，全档（T0 + T1 + T2 + HITL，tracing 可选）。
...
- [接入你的 agent](/zh/guides/connect-your-agent) — 自己写 adapter 时怎么逐档往上爬，每档解锁什么断言。
