---
title: "Run NiceEval in GitHub Actions and CI"
sidebarTitle: "CI Integration"
description: "Add NiceEval to GitHub Actions or any CI. Failed evals exit non-zero, JUnit XML can be emitted, and caching can speed up repeated runs."
---

Evals should live in CI the same way tests do. They catch agent regressions on pull requests and track model, cost, and latency changes in nightly jobs.

## Exit codes

By default, NiceEval exits non-zero when any gate fails. In CI you usually want `--strict` so failure is explicit:

```bash
NICEEVAL_LANG=en npx niceeval exp ci \
  --output ci \
  --strict \
  --json .niceeval/ci-summary.json \
  --junit .niceeval/junit.xml
```

The `ci` profile never prints ANSI, a spinner, or a dynamic table. Its log is a single ordered stdout stream that only appends `start`, low-frequency heartbeats, failures/errors, diagnostics, and the final result — passed attempts are never printed one by one.

## GitHub Actions example

```yaml
name: evals
on:
  pull_request:
  workflow_dispatch:

jobs:
  evals:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
      - uses: actions/setup-node@v4
        with:
          node-version: 22
          cache: npm
      - run: npm ci
      - run: >-
          npx niceeval exp ci --output ci --strict
          --json .niceeval/ci-summary.json
          --junit .niceeval/junit.xml
        env:
          NICEEVAL_LANG: en
          OPENAI_API_KEY: ${{ secrets.OPENAI_API_KEY }}
          ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
```

## Configure secrets

<Steps>
  <Step title="Add repository secrets">
    Put provider tokens into GitHub Actions secrets.
  </Step>
  <Step title="Pass them as env in the workflow">
    Expose only the variables the eval step needs.
  </Step>
  <Step title="Verify they exist">
    Use a minimal eval, or run `list` / a dry run first to confirm configuration.
  </Step>
</Steps>

## JUnit reporter

```ts
import { defineConfig } from "niceeval";
import { JUnit } from "niceeval/reporters";

export default defineConfig({
  reporters: [JUnit(".niceeval/junit.xml")],
});
```

CI can upload the JUnit XML so failures appear in the test-report UI; for a one-off run without touching config, use the CLI flag `--junit <path>` instead. To also report results to an experiment platform such as Braintrust, see [Reporters](/guides/reporters).

## JSON, JUnit, and structured errors

```text
niceeval: start total=24 configs=3 concurrency=10 reused=18
niceeval: errored locator=@2h8m4k1 eval=fixtures/button experiment=ci/codex phase=sandbox-provision reason="E2B sandbox allocation failed after 5 attempts"
niceeval: result=failed passed=23 failed=0 errored=1 reused=18 duration=128s
niceeval: json=.niceeval/ci-summary.json
niceeval: junit=.niceeval/junit.xml
```

The exit code is the first-line red/green signal; JSON, JUnit, and the results snapshot are the complete machine interface — log lines are only for searching and annotation. An `errored` line carries a locator, the eval/experiment identity, the formal phase when known, and a one-line `reason` summary. The full cause, stack, and diagnostics live in the attempt's `result.json` — keep the artifacts and run `niceeval show @<locator>` to review them.

The JSON/JUnit files the CLI is explicitly told to write, and the default results artifact, are all required outputs: a write failure must turn the job red, not just log a warning and exit 0.

## Discovery only

```bash
npx niceeval list
```

This is useful when you only want to confirm that eval files load and configuration is sane.

## Cache `.niceeval/`

You can cache `.niceeval/` in CI, but make sure the fingerprint covers every input that affects the result. For nightly benchmarks, keeping the full artifacts is usually more valuable.

## Control concurrency

```bash
npx niceeval exp ci --output ci --max-concurrency 2
```

Sandbox evals should stay conservative on a standard GitHub-hosted runner. Remote HTTP evals can usually run with higher concurrency if the service allows it.

## Recommended patterns

<CardGroup cols={2}>
  <Card title="PR runs" icon="code-pull-request">
    Run only critical evals and high-risk paths so feedback stays fast.
  </Card>
  <Card title="Nightly full-matrix runs" icon="moon">
    Run the full experiment matrix and record pass rate, cost, and latency trends.
  </Card>
</CardGroup>

Publish results as a static report site (auto-updating on Vercel or GitHub Pages) — see [Publish reports from CI](/guides/publish-report).
