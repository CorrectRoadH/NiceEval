...
- **归因排除清单,runner 私有、锚点时冻结。** 默认排除 `.git`、`node_modules`、常见构建产物与包管理器缓存——不排除的话,`EvalDef.setup` 里一次 `npm install` 会让第一笔 send 前 commit 哈希整棵 `node_modules`,后续窗口的二进制与缓存变化持续放大分类账体积。清单在锚点时冻结,agent 或 fixture 写 `.gitignore` 影响不了它(项目自己的 ignore 规则也**不**参与归因判断——被项目 ignore 的文件照常记录);eval 要评分被排除目录时经 `defineEval({ diff: { include: [...] } })` 显式加回,`diff.ignore` 追加排除。
