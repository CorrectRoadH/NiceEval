...
//
// 断言依据(工具/消息/HITL)全部从协议帧直构,工厂替你做好。这是 Tier 2(send + OTel):
// 比 ../../tier1/ai-sdk-v7 只多一层 OTel delta——本文件加 settleMs、config 加
// telemetry: { port }。OTel 只管 `niceeval view` 的瀑布图:应用官方 @ai-sdk/otel 集成产的
// GenAI span 发到固定端口接收器,应用启动时用标准 OTel 环境变量指过来即可,
// 见 README「跑起来」——span 不喂断言,断言与 Tier 1 完全相同。
