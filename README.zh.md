...
NiceEval 是一个受[eve](https://eve.dev)启发的 agent-native eval 工具。首先有非常优秀的 DX 设计，任何人可以在 10 分钟左右上手并配置。并且设计非常的通用。即可以用来 eval 给 Claude Code/Codex 写的 coding agent 的插件、Hook还有Skill。更可以直接 eval 自己的 AI Agent 应用或框架(无论是基于 AI SDK、LangGraph、Pi 还是自研 agent loop 都可以轻松接入)。
