...
- **留：流程守护。** `test/cases-registry.test.ts`（登记表挂钩——src 测试必须声明所属 `cases.md`、登记表必须有测试实现、`// bug:` 台账引用必须真实）与索引覆盖、链接真实性、生成区块漂移这类守护，守的是"先登记后写测""文档索引不烂"这些**流程契约**——没有任何运行路径会替它们报警，失守只会静默腐坏。
...
- 新功能只新增**已登记场景**的测试：场景行随契约同批定稿在对应 Feature 的用例清单（各 Feature 测试目录下的 `cases.md`）里，测试只实现已登记的行，不顺手改无关断言（先登记后写测，见[用例登记表](unit/registry.md)）；review 时对照清单核对影响面。
- 修 bug 的第一步是把暴露该 bug 的场景登记进对应 Feature 的 `cases.md`——逃逸到真实使用中的 bug 就是登记表的缺口。补场景、写测试、修实现同批完成，memory 台账照记；锁定该修复的测试加 `// bug: memory/<条目>.md` 引用台账条目，让修法从测试可反查现象与根因（规则见[用例登记表](unit/registry.md)）。

每个 Feature 的 `cases.md` 就是这个影响面的登记表：一条契约对应哪些场景，改这条契约时哪些用例要动，一目了然。登记表与测试套件的整册对应关系有机器守护（见[单元测试 · 套件边界与仓库守护](unit/README.md#套件边界与仓库守护)）。
