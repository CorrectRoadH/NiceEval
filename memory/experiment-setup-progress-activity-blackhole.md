...
- 落点:`src/runner/types.ts` + `feedback/{reducer,sink,coordinator,human,agent,ci}.ts` + `run.ts` + `src/i18n/{en,zh-CN}.ts`;测试按 `docs/engineering/testing/unit/experiments-runner/cases.md`「实验级生命周期」新登记的行落在 `reducer/human/agent/ci/run` 五个 test 文件。
