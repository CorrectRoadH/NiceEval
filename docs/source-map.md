...
| `ExperimentComparison`(与其它官方组件同层的普通双面组件,没有渲染器特权;`experimentComparisonData` 对完整 Scope 计算一份 `ScopeSummary`、`MetricScatter`、`ExperimentList`,各 experiment 的 eval 集来自 `selectedEvalIds`) | `src/report/components.tsx`(组件定义,与其它官方组件同文件;web 面 `src/report/react/ExperimentComparison.tsx`) |
