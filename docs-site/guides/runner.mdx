...
npx fasteval exp local weather
```

```bash Tag filter
# Only runs evals tagged "regression"
npx fasteval exp local --tag regression
```

```bash Combined
# Only evals under "fixtures/" tagged "smoke"
npx fasteval exp local fixtures --tag smoke
```
</CodeGroup>

<Tip>
  The positional eval filter appears after the experiment selector. Passing `weather` in `npx fasteval exp local weather` matches `weather/brooklyn`, `weather/tokyo`, and any other eval whose ID starts with `weather`.
</Tip>
