...
- You need the call waterfall (the trace in `niceeval view`): `codex` / `bub` emit OTLP themselves; adapters you write yourself just declare `tracing` (see the OTel guide). `claude-code` currently cannot produce a trace.
