...
`niceeval/report` 的完整 API:React 组件、指标与聚合、计算函数与数据契约、四个 DX 场景,以及把报告页交给 `show` / `view` 渲染的 `defineReport`。整体动机与两档积木的取舍见 [README](README.md);边界、不变量与决策记录见 [Architecture](architecture.md)。
...
换一种看法 = 换积木摆法,数据源一个字不动:把 `MetricTable` 换成 `Scoreboard`、`cell` 从 `passRate` 换成 `examScore`,同一批 artifact 就从 benchmark 榜变成考试成绩单(场景二)。说明文字、布局、品牌色都是你页面里的普通 JSX。
...
结果数据的读与写抽成了专门的库 `niceeval/results`,完整契约在 [Results Lib](../results/library.md);这里只消费它的读取面,备忘入口:`openResults(".niceeval")` 给出「实验 → 快照(单次跑的实验)→ eval → attempt」的类型化层次,外加 `skipped`(读不了的落盘,不静默)与低层的 `runDirs`(忠实磁盘);attempt 级重 artifact(`events` / `trace` / `o11y` / `diff` / `sources`)全部懒加载,缺了返回 `null`;`results.latest()` 返回 **Selection**(快照 + 结构化 warnings 绑在一起),残缺快照的警告替你算好。本文的计算函数统一收 `Selection | Snapshot[]`,其余部分都建立在这些数据之上。
...
`examScore` 仍是「考试」看法的核心积木:gate 是判卷线,soft 是给分点——这套语义 [Scoring](../scoring/README.md) 里本来就有,指标只是把它折成一个数。`tokens` 只加 `inputTokens + outputTokens`:缓存读写量大但便宜,计进去会把缓存热的 agent 画成 token 大户;花钱多少本来就有 `costUSD` 负责。
...
计算函数是「Selection → 一份组件数据」,跑在 Node 侧,产物是**算好的、可序列化的**普通 JSON——终值加渲染提示,不含公式。这与 `niceeval view`「一次性烘焙进静态产物」的哲学一致([View](../view/README.md)):前端(不管是不是我们的组件)只做渲染。
...
**MetricScatter 就是「质量 × 成本 frontier」的积木**:[Experiments](../experiments/library.md#一文件一配置) 的一文件一配置意味着 `compare/bub-low`、`compare/bub-medium`、`compare/bub-high` 各是一个实验——`points: "experiment"` 让每个档位成为一个点,`series: "agent"` 把同 agent 的档位连成线,`better` 驱动的轴向让右上角恒为「又好又便宜」。点的 x/y 就是两个 `MetricCell`:按点维度分组后走同一台两级聚合引擎,所以 `samples` / `total` / `refs` 一应俱全,hover 与下钻不用另做一套。
...
这些**不是持久化格式,只是组件 props**——没有 `format` / `schemaVersion` 信封,也不打版本戳(裁决记录 3,见 [Architecture · 迭代问题裁决记录](architecture.md#迭代问题裁决记录))。要落盘喂 SPA(场景二)就自己包一层 JSON,想加 `generatedAt` 加就是了。兼容性跟随 npm 版本:计算侧和渲染侧本来就是同一个包的两个子路径,同一个应用里天然同版本;唯一的**硬要求**是分离部署(CI 脚本算数据、另一个仓库的前端渲染)时把两边锁在同一个 niceeval 版本。
...
分布类的看法(直方图)不是折叠,计算函数给不了;直接下到结果库的读取面,示例见 [Results Lib · 直接吃读取面](../results/library.md#直接吃读取面一个真实脚本)。
...
定稿采用工厂函数:全家族都是 `define*`,唯独扩展组件要 `class … extends` 是体系里的异类;两个渲染面本质就是两个纯函数,一个对象字面量正好。「少实现一个面编译不过」的强度不降——`web` / `text` 两个渲染面必填,`resolve` 选填(声明了就提供 selection-form,见「数据绑定与 resolve 解析阶段」):
...
现有七个 react 组件(`src/report/react/`)收编为对应双面组件的 web 面,text 面新写;`niceeval/report/react` 的导出保留,供「零件复用」进用户自己的应用(两个导出点上组件都带自己的 `data` 函数)。**配对从 vitest 表升级成结构义务**:`faces` 两键必填,少实现一个面编译不过;双面验收测试(renderToStaticMarkup + text 快照)守护两面对同一份数据给出一致判读——排序方向随 `better`、`samples < total` 角标、缺数据 `—` 不补 0、截断如实报剩余数量。文本渲染住在组件的 text 面里,公开面只有组件。
...
这个字段的语义就是产品 A/B 测试里的 feature flag——一个 experiment 是一组 flag 取值,A/B 域的 multivariate flag 本来就装任意 JSON,「flags 暗示布尔」只在 CLI 域成立;撞词靠行文约定解决:裸词 flags 指实验 flags,命令行开关一律写「CLI flag」或字面 `--xxx`。持久化字段改名是破坏性变更,递增 schemaVersion(现为 3),旧落盘按版本规则进 `skipped("incompatible-version")`,不归一、不迁移——读取边界给旧名做别名就是一次小型迁移,「不解析、不迁移、不猜」无例外;旧结果不丢,`skipped` 携带的 producer 提示 `npx niceeval@<version> view` 正是为这种场景存在。版本序列见 [Results Format](../results/architecture.md)。首批补两个摆法:趋势线 `MetricLine`、分组条形 `MetricBars`(`.data` 为矩阵数据别名),与七个现有产物承担同一套双面义务。

### 类型义务(本提案的落地前置)

公开页承诺「类型化」「编译器背书」,靠以下三处兑现:
...
3. **`EvalResult` 两拆**(持久化瘦身条目 / view 运行时装饰)。`writeAttempt(result, artifacts)` 的第一参是 attempt 级条目——reader 的 `attempt.result` 中,快照级字段(agent / model / startedAt / producer,由 `writer.snapshot()` 一次声明)以外的全部;第二参是懒加载 artifact 的类型——「writeAttempt 参数 + snapshot() 声明 = reader 读回的全部」由类型拼合成立,不需要中间形状。类型迁移步归 [Results Lib](../results/library.md)。
...
- [Experiments](../experiments/README.md) —— 可对比组与 reasoningEffort 档位:scatter 的点从哪来。
- [Scoring](../scoring/README.md) —— gate / soft 断言语义,`examScore` 的依据。
- [CLI](../../cli.md) —— 位置参数 / flag 的 CLI 模型,`--report` 如何嵌入其中。
- [Results Lib](../results/library.md) —— 结果读写库:类型的家、writer、`openResults`、实验/快照层次、选择器、身份键。
