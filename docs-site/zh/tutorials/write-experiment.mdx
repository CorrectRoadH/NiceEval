...
`setup` / `teardown` 只管「在你机器上、一个实验一份」的服务。要在跑 agent 前按实验在**沙箱里**准备环境（装二进制、预热、跨 attempt 载入和回存状态），挂在 `sandbox` 字段的 spec 上：
...
实验级钩子起宿主机侧服务，沙箱钩子每个沙箱把坐标写进去、收尾时回存状态——两层在同一个文件里靠模块变量衔接，时序由 runner 保证：实验级 `setup` 早于本实验任何沙箱钩子，沙箱钩子读到的变量一定已经赋好值：
...
一份实验文件从上往下读就是完整的运行说明：整场一次的宿主机资源在实验级钩子对里；每沙箱的写入与回存在 `sandbox` 链式钩子里，读实验级产物；agent 怎么连自己、eval 的任务夹具各在 agent 定义与 `EvalDef` 里，不进实验文件。

### 多个实验共享同一套生命周期代码

对比组里常常是几个实验对着同一类基础设施——同一个记忆产品，claude 与 codex 各一格对照，起停机制完全一样。把起停写成一个**工厂函数**，返回共享同一闭包的整套件：实验级钩子对、给 agent / MCP 工厂读坐标的 getter、把坐标写进沙箱的 sandbox 钩子。每个实验文件各自调用一次工厂，同一套代码、各自的实例与坐标：
