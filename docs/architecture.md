...
| `t` 暴露什么 | `send`/`reply`/`calledTool`/`judge` | 上述 + `t.sandbox`(沙箱原始句柄)+ 工作区断言(`fileChanged`/`diff`/`judge.agent`,平铺在 `t` 上)+ `t.transcript` |
...
- 声明 `workspace` 能力(沙箱型)→ `t.sandbox`(沙箱原始句柄:命令/文件读写)+ 工作区断言(`fileChanged`/`scriptPassed`/…,平铺在 `t` 上,不在 `t.sandbox` 下)+ `t.diff` + `t.judge.agent` + `t.transcript`(归一化事件 + o11y)。
...
7. **采集生成文件。** `test(t)` 跑完后 `git diff HEAD` 得到生成 / 删除的文件,供 `t.diff` / `t.fileChanged` 读取。
