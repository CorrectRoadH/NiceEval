...
| show / view 内置默认报告(三页普通 `defineReport`:`report` 页 = `Hero` + `ScopeWarnings` + `CopyFixPrompt` + `ExperimentComparison`,`attempts` 页 = `Hero` + `ScopeWarnings` + `AttemptList filter`,`traces` 页 = `Hero` + `ScopeWarnings` + `TraceWaterfall`;页内容全部是公开组件,与 `--report` 同内容文件完全等价) | `src/report/built-in/index.tsx`(默认导出;结构与等价性测试在 `src/report/dual-render.test.tsx`「内建报告」) |
