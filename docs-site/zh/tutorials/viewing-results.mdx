...
超长的工具结果既不整段打印、也不被一刀切丢掉。卡片正文是**有界预览**：保留原始换行，超过 8 KiB（UTF-8 字节）截到预算为止；截断尾巴报被折的字符数，并自带这张卡片的**展开句柄**——整行就是下一条可直接复制的命令。读大文件、全仓 `rg`、curl 整页文档这类动辄几十上百 KB 的结果，预览负责回答「这一步做了什么、开头长什么样」，要看全量再花第二条命令，不用为了防备某张卡片超长就整棵树不敢打开：
...
TOOL · command_execution  5.2s · 1.9s
  ...
  result · completed · exit 0
    ...
    (+263161 chars · niceeval show @1c3h6twx --execution --expand t2.c3)
```
