...
export default defineExperiment({
  ...
  sandbox?: SandboxSpec | SandboxResolver;   // 固定 spec，或按 eval.environment 解析；省略时由 Config.sandbox 兜底
...
`sandbox` 的 resolver 形态只解决**同一 experiment 下不同 eval 需要不同预制环境**：eval 用 `environment` 声明 provider-neutral profile，experiment 再把 profile 映射到 image/template/snapshot。resolver 在调度前对每条选中 eval 求值一次；其结果参与该 eval 的 fingerprint、provider 推荐并发与快照审计。remote Agent 不创建 sandbox，也不调用 resolver。完整写法见 [Library](library.md#按-eval-解析预制环境)。
