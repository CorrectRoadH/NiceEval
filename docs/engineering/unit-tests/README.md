...
1. 它证明了哪一条已声明的功能契约或仓库约束？——并且对应用例清单里的哪一行（见[用例登记表](registry.md)）。
...
2. 确认它在对应 Feature 的用例清单（`<feature>/cases.md`）有登记行——正常流程里场景行在设计阶段已随契约同批定稿；没有的先补行、按契约影响面评审，再动手（流程与预算见[用例登记表](registry.md)）。
3. 写清输入、可观察结果、失败反馈以及不能发生的静默错误。
4. 通过 [Source Map](../../source-map.md) 找到实现该契约的边界。
5. 选择能完整观察该契约的最低层级；只有下层不能证明时才升到组件或集成边界。
6. 覆盖会改变语义的等价类、边界值和状态组合，不按实现分支机械补 case；数量以登记行为界。
...
- **`cases.md`（用例清单）**：契约 → 场景矩阵与示例代码。它是该 Feature 契约影响面的登记表，也是测试数量的闸门——先登记后写测，测试只实现已登记的行；著作权、流程与场景预算见[用例登记表](registry.md)。契约改动时按它核对哪些测试要动，评审测试改动时按它核对影响面没有越界。
...
同一不变量可以在不同故障边界各验证一次，例如领域规则和公共组合——前提是两个边界各自有登记行，且各行写明自己放走的错误类别不同（预算规则见[用例登记表](registry.md)）；写不出不同的错误类别，就只保留一处。
...
- `e2e/repos/**`、`e2e/mechanism/**`：拥有独立执行入口的 E2E 仓库。
...
用例清单本身也在守护范围内：`src/` 下每个测试文件头部用一行注释声明所属清单（`// cases: docs/engineering/unit-tests/<feature>/cases.md`），`test/` 下的守护测试校验两个方向——每条声明指向真实存在的清单，每份 `cases.md` 至少被一个测试文件声明。`test/` 下的仓库守护测试没有 Feature 清单可指，不做此声明，它们的登记表就是自己守护的那条仓库约束（写明在文件头注释里）。机器守护只保证登记表与套件不整册脱钩；逐场景的影响面核对仍是评审对照 `cases.md` 的义务。
