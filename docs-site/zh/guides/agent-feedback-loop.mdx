...
<Steps>
  ...
  <Step title="按问题读取证据">
    ...
    | 时间花在环境还是 agent、卡在哪个生命周期阶段 | `niceeval show @<id> --timing` | 逐生命周期阶段的耗时分解，setup 钩子与每次 send 的步级明细；出错的 Attempt 标出死在哪个阶段 |
...
并按问题选择 --eval、--execution、--timing 或 --diff；--timing 回答时间花在环境还是 agent、
卡在哪个生命周期阶段。写出失败原因的假设，并判断应该修改被测程序、
