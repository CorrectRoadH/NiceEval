...
<Steps>
  ...
  <Step title="Pass secrets as env in the workflow">
    ...
    ```yaml
    - name: Run evals
      run: npx fasteval exp ci --strict
      env:
        ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
        OPENAI_API_KEY: ${{ secrets.OPENAI_API_KEY }}
    ```
...
```bash
npx fasteval exp ci --dry
npx fasteval exp ci memory/ --dry   # check a subset
```
...
npx fasteval exp ci --max-concurrency 2

# Higher value for large runners or cloud sandbox backends
npx fasteval exp ci --max-concurrency 8
```
...
<CardGroup cols={2}>
  <Card title="PR runs" icon="code-pull-request">
    ...
    ```yaml
    run: npx fasteval exp ci --tag smoke --max-concurrency 4
    ```
  </Card>
