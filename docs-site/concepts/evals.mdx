---
title: "Evals in fasteval: lifecycle, outcomes, and eval files"
sidebarTitle: "Evals"
...
```shell
npx fasteval exp local weather          # runs all evals whose ID starts with "weather"
npx fasteval exp local weather/brooklyn # runs exactly this one eval
```
...
<Steps>
  <Step title="Discovery">
    When you run `npx fasteval exp ...`, the runner recursively scans the `evals/` directory for files ending in `.eval.ts` and for fixture directories (those containing a `PROMPT.md`). Default exports of `defineEval(...)` or an array of `defineEval(...)` calls are registered as individual evals.
  </Step>
