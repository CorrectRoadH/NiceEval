...
`src/runner/run.ts` 引入 `multiOpenSeen`:判定「这个结果根上还有别的 Invocation 在动」就在
**全新取锁**这条路径上也重查一次携带。判据是几个便宜的信号叠加——启动时锁目录非空 / 取锁前
读到过别人的记录 / 撞过锁 / 接管过 / 实验闸名额被别人占满等过。单开只付启动时一次 `readdir`,
最常见的路径不变贵(重查要 `loadLatestResultsPerEval` 全树扫描,不能每条用例无条件做)。

残留窗口:并行 Invocation 全程与我们零碰撞(它在我们启动扫描之后开始、在我们派发那条重叠
用例之前结束)——此时代价是一条用例被双跑,可接受。要彻底闭合只能每次派发前都重查,或者
给结果树加一个便宜的「自我上次读盘以来变过没有」水位标记。
