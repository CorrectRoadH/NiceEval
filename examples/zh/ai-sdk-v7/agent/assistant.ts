// 被测助手:系统提示 + 四个工具 + 一个 chat 函数。纯 AI SDK v7 工具循环,不 import 任何
// eval 框架的东西 —— 接入 niceeval 后应用侧唯一的改动是 chat 多收一个可选 opts
// (取消信号 + telemetry),两个字段都原样交给 generateText。
//
...
export interface ChatOptions {
  ...
  signal?: AbortSignal;
  /** generateText 的 telemetry 选项:观测 / eval 场景由调用方注入,不设则不埋点。 */
