...
describe("TTY dashboard: 静态符号,不用 spinner 帧驱动重画", () => {
  it("active 行恒用静态 ● 号,从不出现任何 spinner 字符", async () => {
    ...
    coordinator.emit({ type: "attempt:start", at: 0, identity: ref("memory/a"), who: "bub-e2b", phase: "running" });
    fake.advance(1000);
    await flush();
...
describe("TTY dashboard: 高度以 stderr.rows 为硬上限,窄终端先减 active slots", () => {
  it("active attempt 数超过可视预算时,先折叠成更少的行 + 一条 overflow 摘要,总行数不超过 rows", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      coordinator.emit({ type: "attempt:start", at: 0, identity: ref(`memory/eval-${i}`), who: `who-${i}`, phase: "running" });
    }
    fake.advance(250);
    await flush();
...
describe("TTY dashboard: active slots 稳定,完成前不因其它 attempt 更新而换位", () => {
  it("A 完成、C 加入后,B 仍保持在 C 之前(不重新排序成到达/字母序)", async () => {
    ...
    coordinator.emit({ type: "attempt:start", at: 0, identity: ref("memory/a"), who: "who-a", phase: "running" });
    coordinator.emit({ type: "attempt:start", at: 0, identity: ref("memory/b"), who: "who-b", phase: "running" });
    fake.advance(250);
    await flush();

    coordinator.emit({ type: "attempt:complete", at: 0, identity: ref("memory/a"), who: "who-a", verdict: "passed" });
    coordinator.emit({ type: "attempt:start", at: 0, identity: ref("memory/c"), who: "who-c", phase: "running" });
    fake.advance(250);
    await flush();
