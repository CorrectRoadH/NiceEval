...
英文术语描述组件形态，API 名是代码里的导出名。默认组合件比较当前 Scope；实体列表逐项展示 experiment、评估用例或 Attempt；汇总组件概括整批 Selection；指标图形把指定维度聚合成值。
...
`niceeval show` / `view` 不传 `--report` 时使用的默认组合件。它直接计算当前 Scope 的摘要、成本 × 端到端通过率散点和实验列表：

```tsx
<ExperimentComparison data={await ExperimentComparison.data(selection)} />
```

每个 experiment 的评估用例集合来自快照 `selectedEvalIds`。网页面与终端面都显示完整 Scope；需要子集时用 `--exp` 或先对 Selection 调 `filter`。
...
下面的 `MetricScatter`、`MetricTable` 和 `ExperimentList` 同样忠实消费调用方传入的数据，不推导隐藏范围。
...
每项固定代表一个 experiment。主行显示 experiment id、agent、model、flags、评估用例判定构成、通过率、Tokens、成本和耗时。默认 `ExperimentComparison` 把当前 Scope 的全部 items 交给它；组件本身不猜边界。
...
直接把 `selection` 传给它，宿主渲染前替你算好数据。`MetricScatter` 和默认 `ExperimentComparison` 都直接消费调用方给出的 Scope。要嵌预先算好的数据，改传 `data`。
