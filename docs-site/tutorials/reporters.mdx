...
[NiceEval](https://niceeval.com/) runs and scores evals itself. Reporters send completed results elsewhere. Feedback while a run is in flight (Human/Agent/CI) is chosen with `niceeval exp --output ...`, not a user-configured Reporter; the `.niceeval/` results artifacts are always on. Other reporters are imported from `niceeval/reporters` and mounted as needed.
