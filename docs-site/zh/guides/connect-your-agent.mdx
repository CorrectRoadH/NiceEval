...
- **Tier 2:侵入**。要评「改 prompt / 换工具集 / 开关某个 feature 谁更好」这类 **feature A/B test**,对照点在应用内部,无侵入拿不到——需要变更内部代码,把变体暴露成外部可选的配置(经 experiment 的 `flags` → `ctx.flags` 透传),adapter 仍然黑盒调用应用的对外接口。
...
<CardGroup cols={3}>
  <Card title="AI SDK 应用" icon="bolt" href="/zh/reference/builtin-agents">
    用 Vercel AI SDK（`useChat` 后端）写的应用：内置 `uiMessageStreamAgent` 黑盒接它的 HTTP 接口，零映射、含 HITL，有完整的 before/after 示例。不用读本文。
  </Card>
...
写自己的 adapter 时可以对着[内置 Agent 能力](/zh/reference/builtin-agents)逐项核对行为。完整的可跑参考：五个 [Tier 1 接入示例](/zh/example/tier1-ai-sdk-v7)，全部黑盒接入，覆盖事件流断言、多轮隔离、HITL 批准 / 拒绝、OTel 派生、trace。

黑盒 adapter 手写 `send` 时，真正要写的只有"怎么发"和"帧类型 → 事件"这张映射表——会话续接（`resumeId` / `clientHistory`）、HITL 暂停恢复（`pausable`）、逐帧驱动的循环（`driveFrameStream`）都是官方件，不用每接一个新后端都重写一遍，见[内置 Agent 能力参考](/zh/reference/builtin-agents)里的通用拼装件一节。
