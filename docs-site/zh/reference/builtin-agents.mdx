...
- 支持 `maxTurns` 配置项：限制最多跑几个 tool-use 轮次（→ `--max-turns`），省略时用 CLI 原生默认（无限制）。
...
`uiMessageStreamAgent(options)` 除 `name`/`url`/`body`/`denyReason` 外还接受：

- **`headers`**：附加请求头（鉴权等），对象或 `(ctx) => Record<string, string>` 函数形式；`ctx.telemetry.headers`（traceparent）总会自动并入，不需要自己拼。
- **`settleMs`**：流结束后再等这么久才返回（毫秒），给应用侧的观测导出（如 `BatchSpanProcessor`）留时间，不设置就不等待。
- **`tracing`** / **`spanMapper`**：应用有 OTel 时的端点投递方式与 span 归一函数（拿瀑布图用）；事件流本身始终从协议帧直构，不依赖这两项。
