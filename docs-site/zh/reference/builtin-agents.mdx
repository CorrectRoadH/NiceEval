...
各 agent SDK 的流式协议是 SDK 定义的通用协议，不是某个应用的私有格式——原生帧 → 标准事件的映射知识在官方包里（`niceeval/adapter` 导出），自己写黑盒 adapter 时只剩**传输粘合**（应用把流放在哪个端点、审批走什么端点）：
...
for (;;) {
  ...
  if (frame === null) break;
  events.push(...stream.add(frame));                  // 认不出的帧返回 []
}
...
可跑参考：[Tier 1 接入示例](/zh/example/tier1-claude-sdk)里 claude-sdk / pi-sdk / codex-sdk 三个 adapter 就是这三个转换器 + 传输粘合。
...
- 需要人工审批 / 多步确认（HITL）：`uiMessageStreamAgent` 原生支持（AI SDK v7 tool approval）；沙箱型三个都不支持，其它被测对象自己写 adapter（Tier 1 示例里 pi-sdk / claude-sdk 有现成写法）。
