...
turn.data;      // 结构化产物:应用有结构化输出、adapter 填了才存在,类型是 unknown(见下)
...
应用有结构化输出（JSON 回复、AI SDK 的 `experimental_output` 这类）时，adapter 把它放进 `turn.data`，用 `turn.outputMatches(schema)`（Standard Schema，zod 即可）或 `turn.outputEquals(value)` 断言它：

```ts
const turn = await t.send("这张发票总额多少?");
turn.outputMatches(z.object({ amount: z.number(), currency: z.string() }));
turn.outputEquals({ amount: 3200, currency: "USD" });
```

`turn.data` 的类型是 `unknown`——要在 eval 代码里直接读字段，自己收窄。它不是"agent 顺手返回点什么"的口袋：只回文本的应用不该碰这个字段，形状断言交给 `outputMatches`。
...
`t.sendFile(path, text?)` 读一个本地文件(相对 eval 所在目录),按扩展名推断 MIME 类型,以 base64 编码的 `InputFile`（`{ filename, mimeType, dataBase64 }`）附加到这一轮的输入里：
...
`t.requireInputRequest(filter)` 把一个待处理的 HITL 请求变成可检查、可回应的具体值——如果匹配到 0 个或超过 1 个待处理请求就会抛出,所以尽量把能填的 filter 字段都填上(`id` / `prompt` / `display` / `action` / `optionIds` / `input`)来消歧。`t.respond(...)` 回答它并发出下一轮;每个参数是一条回答,按待处理请求的顺序对位。在底层它只是又一次普通的 `send`:回答以纯文本进 `input.text`(多条按 `"\n"` 拼接),adapter 按自己上一轮吐出 `input.requested` 的顺序对位、自己解释文本。每种回答到 adapter 长什么样,见[回答怎么到达](/zh/concepts/agents-adapters#回答怎么到达)。

如果当前轮有多个同类待处理请求、都该给同一个答案(比如逐个批准一批文件改动),用 `t.respondAll(optionId)` 一次性处理,不用挨个解。eval 侧只校验"确实有待处理请求";`optionId` 本身不校验,打错的字会原样传给应用：
