...
Example: &#123; name: "browser", command: "npx", args: ["-y", "@anthropic/mcp-browser"] &#125;
...
Extra skills to install, in GitHub `"org/repo"` form (e.g. `"Effect-TS/skills"`).
During setup, runs `npx skills add <org/repo>` in the sandbox;
the result is written to the sandbox workspace's skills-lock.json, which the claude CLI reads automatically on startup.
...
The shape corresponds to codex config.toml's [mcp_server.&lt;name&gt;] table.
...
Extra skills to install, in GitHub `"org/repo"` form (e.g. `"Effect-TS/skills"`).
Runs `npx skills add <org/repo>` during setup, and the result is written to skills-lock.json.
...
Extra Python packages to install into the bub tool environment (pip names or git URLs).
On each sandbox setup, appended into the bub environment as `uv tool install --with <pkg>`.
Example: ["bub-plugin-memory", "git+`https://github.com/..."`]
