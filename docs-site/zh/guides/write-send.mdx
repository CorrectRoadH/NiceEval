---
title: "写 send：从一条消息到完整接入"
sidebarTitle: "写 send"
description: "手写 Adapter 的 send 函数，一条主线六步走：发消息拿回复、接上之前的消息、把工具解析成事件、HITL 停轮续跑、接上 OTel trace、透传 experiment 的 flags。每一步只在上一步的代码上多几行，每一步解锁一组断言。"
---
...
但这段循环通常**不用你写**。返回是标准形状时，官方转换器一行顶替上面全部——`events`、`usage`、`status` 都在返回值里，拿来直接 `return`：

```ts
    // ……transport 与会话同第二步，省略
    return fromChatCompletion(res);            // ← 上面那段映射的官方版
```
...
| `input.requested`（配合 status `"waiting"`） | `t.parked()`、`t.requireInputRequest()`、`t.respond()`（第四步） |
...
HITL 几乎总是发生在流式接口上（停在流中间），所以这一步的示例换成一个以 SSE 透传原生事件的应用——它同时用上前三步的全部内容（完整可跑版本见 [tier1 示例](https://github.com/CorrectRoadH/niceeval/tree/main/examples/zh/tier1)）：
