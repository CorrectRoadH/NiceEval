...
| `workspace.baseline` | preparing workspace | 建立 git 空基线(diff 的起点);remote agent 跳过 |
...
```text
NICEEVAL result locator=@12p9k4mz verdict=passed attempts=1 planned=3 unstarted=2 reason=early_exit
```

`--no-early-exit` 跑满后使用真实分母:

```text
NICEEVAL result locator=@12p9k4mz verdict=passed attempts=3 passed=2 rate=0.667
```
