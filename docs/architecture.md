...
12. **报告。** 每个 eval 完成即在串行报告队列上回调 `onEvalComplete`(不阻塞执行池),对应 attempt 的判决与 artifact 随之写进该实验快照目录(`.niceeval/<experiment>/<snapshot>/<evalId>/aN/result.json`);全部完成后回调 `onRunComplete`,快照收尾补 `snapshot.json` 的 `completedAt`。
