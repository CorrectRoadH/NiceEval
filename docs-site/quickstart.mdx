...
description: "Install niceeval, scaffold your project, and run your first direct-agent and coding-agent evals in under 10 minutes."
---

This guide takes you from a blank project to a runnable niceeval setup: one direct agent eval and one coding-agent eval that runs inside a Docker sandbox. By the end you'll have an eval suite with CI-ready output.
...
<CardGroup cols={3}>
  ...
  <Card title="Eval an AI agent application" icon="globe" href="/example/ai-agent-application">
    For AI SDK agents, LangGraph, or any custom agent application.
  </Card>
</CardGroup>
...
## Eval 1 — a direct agent

The fastest way to start is evaluating an agent you can call directly from your own codebase. You define an agent adapter that invokes your runtime, then write an eval that checks the output. No Docker required.

First, create the agent adapter:

```ts
// agents/classify.ts
import { defineAgent } from "niceeval/adapter";
import { runAgent } from "../src/agent.js"; // your own agent runtime

export default defineAgent({
  name: "classify",
  async send(input) {
    return { data: await runAgent(input.text), status: "completed" };
  },
});
```
...
<Tip>
  Direct evals are ideal for semantic regression testing. Because they can call your agent runtime without a sandbox, they stay fast and slot naturally into a standard CI pipeline.
</Tip>

## Eval 2 — a deployed or remote agent

To eval an agent that runs outside the current process, write an adapter that handles the request/response cycle. Connection details and credentials are the adapter's concern — niceeval has no `--url` flag and imposes no protocol requirements.

Define the agent adapter:
...
Run it by passing your agent URL as an environment variable:
