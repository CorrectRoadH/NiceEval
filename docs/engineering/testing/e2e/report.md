...
判定、聚合、计算口径、装载校验与错误反馈这些**数据语义**归[单元测试 Reports](../unit/reports/README.md)——`*Data` 函数与 resolve 管线在 fixture 上证明，不需要真实运行。本仓库承接从数据到呈现的一切：渲染出来的结构、排版、样式与交互，以及 CLI 读面的进程级行为。
