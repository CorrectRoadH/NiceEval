...
Human active 行的最后一栏显示正式 `AttemptPhase`,不能直接把 `AgentContext.log`(`ctx.log(...)`)的最后一条自由文本当阶段。阶段按生命周期单向推进:
...
`AttemptPhase` 是 runner 对真实 lifecycle 的 UI 投影,不是 adapter、sandbox provider 或用户 hook 能直接设置的公共字段。上一节表格里的每一次转换都由 `attempt.ts` 沿它自己固有的执行顺序、在真正跨入该步骤时发出;没有对应 hook/配置的步骤直接跳过,不产生空阶段。各层已有的自由文本进度(`AgentContext.log`,即 eval/adapter 里 `ctx.log(...)` 这一路调用)继续原样工作:一句话原样挂到 runner 当前认定的 phase 上作为可选 `detail`,只更新 Human dashboard 当前 active 行的次要文本,`agent` / `ci` profile 不展示,也不写入 results;它不能改写 phase 本身,也不能声称进入了另一个生命周期阶段。

Sandbox provider、`SandboxSpec.setup/teardown` hook、`EvalDef.setup/test`、Agent adapter、Telemetry、Scoring 这六类扩展点今天没有各自独立的、按 owner 隔离的结构化 progress/diagnostic 出口——想表达"我正在做什么"只能复用 `AgentContext.log` 或让异常/返回值自然传播,由 runner 统一归并进当前 phase。按 owner 拆分 operation scope、给每一类扩展点发一份专属 `progress()` / `diagnostic()` 句柄是候选的后续设计,见 [Scoped Attempt Feedback](../../roadmap/scoped-attempt-feedback.md)。
...
NICEEVAL error locator=@2h8m4k1 eval=memory/agent-029-use-cache experiment=compare/claude-e2b phase=sandbox-provision verdict=errored
...
niceeval: errored locator=@2h8m4k1 eval=memory/agent-029-use-cache experiment=ci/claude-e2b phase=sandbox-provision reason="E2B sandbox allocation failed after 5 attempts"
...
agent  NICEEVAL error locator=@8c1m2q eval=memory/agent-029 experiment=compare/bub-e2b phase=running verdict=errored
ci     niceeval: errored locator=@8c1m2q eval=memory/agent-029 experiment=compare/bub-e2b phase=running reason="attempt timed out (60000ms)"
```
