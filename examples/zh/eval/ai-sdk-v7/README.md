...
- `src/backend/tools.ts` 多一个 `sendEmail` 纯函数(`send_email` 工具的实现)。
- `src/backend/models.ts`、`src/backend/server.ts`、`src/frontend/`、`index.html`、`vite.config.ts` 与
  origin/ai-sdk-v7 **逐字节相同**——同一个聊天应用,能不能接 eval 跟这些代码没关系。
...
也可以像 origin/ai-sdk-v7 一样起本地聊天应用(手动试聊天,不跑 eval):
