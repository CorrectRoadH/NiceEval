...
多轮 `t.send()` 要求 Agent 定义里递了会话件(`session: clientHistory()` 或 `serverSession()`)——没递时,adapter 没有续接上一轮的手段,第二次 `t.send()` 会直接报错而不是静默接进一个无关上下文。能力如何从构造中来,见 [Adapter](/zh/concepts/adapter)。
