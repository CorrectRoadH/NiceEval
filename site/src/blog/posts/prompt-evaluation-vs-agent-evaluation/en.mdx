...
Typical metrics include answer relevance, factual accuracy, and format compliance, sometimes with toxicity or hallucination detection layered on top. A mature set of tools has grown up around this paradigm — DeepEval, RAGAS, LangFuse — and they all do the job well, as long as all you need is to evaluate a prompt.
...
| Typical tools | DeepEval, RAGAS, LangFuse | NiceEval |
...
NiceEval is an open-source project built by CorrectRoadH, aimed squarely at the gap prompt evaluation tools can't reach: once your system stops being a black box and becomes a chain of tool calls, multi-turn conversations, and state transitions, what you need isn't a stronger prompt scorer — it's an evaluation framework that can trace the full decision path.

## How to choose

If your product today is a single-turn chatbot, a text classifier, or a summarizer, prompt evaluation tools are enough. DeepEval or RAGAS, plus a handful of test cases, will cover your quality needs.
