...
- [报告域](results.md)——`results` 仓库：落盘格式与公开读取面。
- [CLI 域](cli.md)——`cli` 仓库：选择、退出码折叠与缓存复用。
...
4. **正反路径可证**：支持某项能力的仓库同时验证正例和反例；退出码折叠的验证由 [CLI 域](cli.md)的 mechanism 仓库承担，其 deliberate-fail / deliberate-error 实验把预期非零退出转换为仓库级验收成功。
...
| `group` | 调度分组：`sdk`、`sandbox`、`mechanism` |
...
`openResults()` 读取库是公开使用面的一部分，它的 E2E 验收归[报告域](results.md)的 `results` 仓库——只有那里可以断言落盘格式与读取库，其它仓库不复制格式知识、不经库面读结果。
...
读回不断言终端布局、列格式或文案——渲染语义归[单元测试 Reports](../unit-tests/reports/README.md)；也不断言完整落盘与出口格式——那是[报告域](results.md)`results` 的责任。这条边界让读回的维护成本停留在「自有事实的子串级检查」，矩阵修复成本不随格式微调放大。
...
`repos/` 与 `mechanism/` 是 §2.1 定义的两个 collection 目录；新增 collection 只是给未来某类仓库多一个物理归处，不改变发现协议本身。
...
| 层级 | 内容 | 触发 |
|---|---|---|
| PR | SDK 仓库与 mechanism 仓库的便宜档 | pull request、main push |
...
1. **mechanism 仓库先行**（`results`、`cli`）：它们最薄、断言只依赖机制事实，先绿说明新契约本身自洽。
