...
describe("start 立即追加;仅连续 60 秒无永久事件才 heartbeat;failure 后重置时钟", () => {
  it("cli.md「CI 怎么用」给出的字面例子:逐行复现", async () => {
    ...
    for (let i = 0; i < 6; i++) {
      coordinator.emit({ type: "attempt:start", at: 100, identity: ref(`memory/eval-${i}`), who: "ci/bub", phase: "running" });
    }
...
describe("passed 不逐条打印;failed/errored 立即打印 locator;默认展开上限 50", () => {
  it("passed attempt 的 lifecycle 事件不产生任何额外 stdout 输出", async () => {
    ...
    await flush(); // "plan" 的 "start" 行是异步队列任务,先让它落地再取基线计数
    const afterStart = fake.stdout.writes.length;
    coordinator.emit({ type: "attempt:start", at: 0, identity: ref("memory/a"), who: "ci/bub", phase: "running" });
    coordinator.emit({ type: "attempt:phase", at: 1, identity: ref("memory/a"), phase: "scoring" });
...
describe("集成测试覆盖 cli.md「CI 常见 case」的三条命令", () => {
  it("PR 快速门禁(--runs 1 --junit ...):只出 junit,首过即停省略数不计入 unstarted/退出码", async () => {
    ...
    for (let i = 0; i < 6; i++) {
      coordinator.emit({ type: "attempt:start", at: 0, identity: ref(`memory/eval-${i}`, 0, "pr/bub"), who: "pr/bub", phase: "running" });
    }
    for (let i = 0; i < 5; i++) {
