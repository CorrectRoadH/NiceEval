...
7. 登记表新行：`docs/engineering/testing/unit/sandbox.md`「孤儿核对与 prune」、`docs/engineering/testing/unit/experiments-runner.md` 实验级生命周期新增 4 行 + 缓存分区新增 2 行——测试只为这些行而写。
...
- 验收：`docs/engineering/testing/unit/sandbox.md`「孤儿核对与 prune」3 行全部变绿；`pnpm run typecheck`、`pnpm test` 通过；本机 docker 手测：起一个假 niceeval label 容器（pid 填已死进程）→ `list --orphans` 可见 → `prune` 收回。
