...
E2E 的边界是**公开可观察契约**，不是“只能看屏幕像素”。它可以断言 CLI
的参数、退出码、stdout/stderr、公开文件格式、HTTP 响应、浏览器里的语义结构与交互，
以及文档声明的 package API；但必须从候选包的公开入口进入，不能 import 内部 renderer、
组件或 `*Data` 来替用户走捷径。预期值是签入测试仓库的 Eval、fixture 与公开文档共同
给出的独立 oracle，不能从候选实现导入常量或调用候选算法现算——否则实现和预期会一起错，
测试仍然全绿。
