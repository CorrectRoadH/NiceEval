...
| --- | --- |
| 默认报告（总览、每组的散点图与榜单，每行可点开看这个实验下每道题的判定与原因） | `niceeval show` |
...
不带 flag 的 `niceeval show` 渲染默认报告的终端版，从上到下两块：运行总览（几个 experiment、几个 eval、判定分布、总成本、合成来源），每个实验组一节（组按 experiment id 的 `/` 前缀划分）——组内一张质量 × 成本字符坐标图加一张榜单；榜单每行是一个 experiment，有失败/错误题的行下面跟一段缩进明细（哪道题、什么原因、下钻命令）：
...
榜单每行一个 experiment，带 agent 与 model 列——同组多配置并排时一眼看清每行是谁；verdicts 是按 eval 折叠后的判定计数。跑多 experiment 的组时，`niceeval exp` 收尾块里的汇总表就是这个榜单的形状——同一份表格渲染，判定折叠口径也相同；区别只在数据范围：收尾块只汇总本次 run，`show` 是跨 run 合成的现刻水位，各自在表头如实标注。网页版（`niceeval view`）同一份数据，榜单每行可以点开/收起这段明细，不用一直全部铺开。
...
view 的首页是一份报告：不传 `--report` 时是默认报告——与 `show` 同一份内容（总览、每组的散点图与榜单）的网页版；传了 `--report` 就换成你自己的报告（与 `show --report` 吃同一个文件），见[自定义报告](/zh/guides/custom-reports)。证据部分（attempt 弹窗、transcript、trace 瀑布、run 列表）始终保留，报告里的数字点进去就是证据。
