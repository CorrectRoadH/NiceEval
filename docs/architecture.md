...
- 声明 `workspace` 能力(沙箱型)→ `t.sandbox`(沙箱原始句柄:命令/文件读写 + 工作区断言 `fileChanged`/`scriptPassed`/`diff`/…,来源 Vercel agent-eval)。评工作区产物用 `t.judge.autoevals.closedQA` 配 `{ on: t.sandbox.diff.get(path) }`。
