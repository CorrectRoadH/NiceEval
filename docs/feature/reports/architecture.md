...
- **管线以 page 实例为单位执行。** scope-input page 各有一个实例；attempt-input page 对每个可达 locator 各形成一个实例。本地 view 只 resolve 被打开的实例，某实例失败时显示完整错误反馈而不污染其它实例；静态导出在写文件前 resolve 和校验全部 scope pages 与全部可达 locator 实例，任一失败都整体失败。外壳、page id、输入声明与导航资格在装载期先校验；content 在 resolve 展开时逐节点校验。
