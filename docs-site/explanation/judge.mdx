...
Judge calls are scoring functions like any other assertion, so they follow the same mechanics described in [Assert · Gate vs soft severity](/concepts/assert#gate-vs-soft-severity) — but their **default** is different from most value matchers:
...
- [Assert](/concepts/assert) — gate vs soft severity in full, plus the verdict rules judge scores feed into.
- [Drive](/concepts/drive) — `t.send()`, `t.newSession()`, and which handle `t.judge`, `session.judge`, and `turn.judge` each hang off of.
- [Evals](/concepts/evals) — how judge scores fold into the eval lifecycle and verdict types.
