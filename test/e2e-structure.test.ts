...
// 这里守的是 README §8 列出的结构边界——它是仓库形状的契约测试,不是某个仓库的验收测试
// (仓库自己的验收脚本见 verification.md)。仓库集合会增删,断言基于 readdirSync 动态发现,
// 不写死仓库名单;零仓库(collection 目录缺失或为空)时循环体不执行、数组天然为空。
...
// README §2.1/§8:每个仓库带一份只含空 `packages: []` 的 pnpm-workspace.yaml,让 pnpm 把
// 仓库目录本身当 workspace root、不向上并入父级 workspace——注入的候选 tarball 才会解析到
// 仓库自己的 node_modules。文本扫描(不引 YAML 解析器):容忍 "packages: []" 与
// "packages: [ ]" 的空白差异,以及 flow 数组同行写法;非空 packages(如 "packages:\n  - x")
// 不匹配这条正则,判违规。
function declaresEmptyWorkspacePackages(content: string): boolean {
  return content.split("\n").some((l) => /^\s*packages:\s*\[\s*\]\s*$/.test(l));
}
...
function repoStructureViolations(repoId: string, repoRoot: string): string[] {
  ...
  if (!existsSync(workspacePath)) {
    violations.push(
      `[${repoId}] 缺少 pnpm-workspace.yaml——仓库需用空 \`packages: []\` 自成 workspace root,` +
        `否则就地 pnpm install 会向上并入父级 workspace、绕过候选 tarball 注入(README §2.1/§8)`,
    );
  } else if (!declaresEmptyWorkspacePackages(readFileSync(workspacePath, "utf8"))) {
    violations.push(
      `[${repoId}] pnpm-workspace.yaml 没有空的 \`packages: []\`——仓库必须自成 workspace root,` +
        `不能声明成员包并入父级 workspace(README §2.1/§8)`,
    );
  }
  return violations;
}
...
const FORBIDDEN_E2E_ADAPTER_IMPORTS = new Set([
  "defineAgent",
  "defineSandboxAgent",
  "driveFrameStream",
  "deltaStream",
...
/** README §2.1:启用仓库只能直接实例化官方 Agent 工厂,不能保留本地 Adapter 实现层。 */
function repoOfficialFactoryViolations(repoId: string, repoRoot: string): string[] {
  const violations: string[] = [];
  if (existsSync(join(repoRoot, "agents"))) {
    violations.push(
      `[${repoId}] 存在顶层 agents/——启用 E2E 只能在 Experiment 中直接实例化官方 Agent 工厂;` +
        `缺少完整工厂的 fixture 应移入 e2e/undo(README §2.1/§2.2)`,
    );
  }

  const files = walkFiles(repoRoot).filter((f) => SOURCE_EXTENSIONS.some((ext) => f.endsWith(ext)));
  const namedImport = /\bimport\s*\{([\s\S]*?)\}\s*from\s*["']niceeval\/adapter["']/g;
  const namespaceImport = /\bimport\s*\*\s*as\s+\w+\s*from\s*["']niceeval\/adapter["']/;

  for (const file of files) {
    const text = readFileSync(file, "utf8");
    if (namespaceImport.test(text)) {
      violations.push(
        `[${repoId}] ${relative(ROOT, file)} 用 namespace import 引入 niceeval/adapter——` +
          `结构守护无法证明它只调用官方工厂,请改为具名导入`,
      );
    }
    for (const match of text.matchAll(namedImport)) {
      ...
      for (const name of imported) {
        if (!FORBIDDEN_E2E_ADAPTER_IMPORTS.has(name)) continue;
        violations.push(
          `[${repoId}] ${relative(ROOT, file)} 从 niceeval/adapter 导入 ${name}——` +
            `启用 E2E 不得拼装自定义 Agent;补齐官方工厂后直接实例化,否则移入 e2e/undo`,
        );
      }
    }
  }
...
describe("e2e 测试仓库结构守护(docs/engineering/e2e-ci/README.md §2 / §8)", () => {
  ...
  it("每个仓库都有自己的 package.json、lockfile、pnpm-workspace.yaml(packages: [])、evals/、experiments/、.env.example,以及忽略 .niceeval/ 的 .gitignore", () => {
  ...
  it("启用仓库没有 agents/，也不导入自定义 Agent 拼装入口", () => {
    const violations = repos.flatMap(({ id, dir }) => repoOfficialFactoryViolations(id, dir));
    expect(violations, `\n${violations.join("\n")}`).toEqual([]);
  });
});

// ---------------------------------------------------------------------------
// 根编排脚本守护——list.ts / run.ts 不得内置 SDK/仓库专属知识
// ---------------------------------------------------------------------------
...
describe("根编排脚本不内置 SDK/仓库专属知识(README.md §5 / §8)", () => {
  ...
  it("list.ts / run.ts(存在时)不含 adapter factory 名、MCP 工具名前缀或写死的 eval 数量断言", () => {
    ...
    for (const scriptPath of scriptPaths) {
      if (!existsSync(scriptPath)) continue; // 脚本缺失时对它平凡通过(守的是内容,不是存在性)
...
describe("守护函数自测(合成样本,证明规则真的会咬人)", () => {
  ...
  it("declaresEmptyWorkspacePackages 只认空 packages: [],拒绝非空成员列表与缺失", () => {
    expect(declaresEmptyWorkspacePackages("packages: []\n")).toBe(true);
    expect(declaresEmptyWorkspacePackages("packages: [ ]\n")).toBe(true);
    expect(declaresEmptyWorkspacePackages("# comment\npackages: []\nallowBuilds:\n  esbuild: true\n")).toBe(true);
    expect(declaresEmptyWorkspacePackages("packages:\n  - 'apps/*'\n")).toBe(false);
    expect(declaresEmptyWorkspacePackages("allowBuilds:\n  esbuild: true\n")).toBe(false);
    expect(declaresEmptyWorkspacePackages("")).toBe(false);
  });
  ...
  describe("对文件系统的结构/依赖检查(临时目录合成仓库)", () => {
    ...
    it("repoStructureViolations 在完整仓库上通过、在残缺仓库上逐项报缺失", () => {
      ...
      writeFileSync(join(complete, "pnpm-workspace.yaml"), "packages: []\n");
      ...
      expect(violations.some((v) => v.includes("pnpm-workspace.yaml"))).toBe(true);
      ...
      writeFileSync(join(gitignoreNotIgnoring, "pnpm-workspace.yaml"), "packages: []\n");
      ...
      const nonEmptyWorkspace = join(scratch, "member-workspace-repo");
      mkdirSync(join(nonEmptyWorkspace, "evals"), { recursive: true });
      mkdirSync(join(nonEmptyWorkspace, "experiments"), { recursive: true });
      writeFileSync(join(nonEmptyWorkspace, "package.json"), "{}");
      writeFileSync(join(nonEmptyWorkspace, "pnpm-lock.yaml"), "lockfileVersion: '9.0'\n");
      writeFileSync(join(nonEmptyWorkspace, "pnpm-workspace.yaml"), "packages:\n  - 'apps/*'\n");
      writeFileSync(join(nonEmptyWorkspace, ".env.example"), "");
      writeFileSync(join(nonEmptyWorkspace, ".gitignore"), ".niceeval/\n");
      const memberViolations = repoStructureViolations("member-workspace-repo", nonEmptyWorkspace);
      expect(memberViolations).toEqual([
        expect.stringContaining("没有空的 `packages: []`"),
      ]);
    });
    ...
    it("repoOfficialFactoryViolations 拒绝 agents/ 和转换器拼装,放行官方工厂", () => {
      const invalid = join(scratch, "custom-adapter-repo");
      mkdirSync(join(invalid, "agents"), { recursive: true });
      writeFileSync(
        join(invalid, "agents", "custom.ts"),
        'import { defineAgent, fromLangGraphEvents } from "niceeval/adapter";\n',
      );
      const invalidViolations = repoOfficialFactoryViolations("custom-adapter-repo", invalid);
      expect(invalidViolations.some((v) => v.includes("顶层 agents/"))).toBe(true);
      expect(invalidViolations.some((v) => v.includes("defineAgent"))).toBe(true);
      expect(invalidViolations.some((v) => v.includes("fromLangGraphEvents"))).toBe(true);

      const valid = join(scratch, "official-factory-repo");
      mkdirSync(join(valid, "experiments"), { recursive: true });
      writeFileSync(
        join(valid, "experiments", "ci.ts"),
        'import { claudeCodeAgent } from "niceeval/adapter";\nexport const agent = claudeCodeAgent({});\n',
      );
      expect(repoOfficialFactoryViolations("official-factory-repo", valid)).toEqual([]);
    });
  });
});
