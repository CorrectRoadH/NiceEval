...
//   3. 会话续接(resumeId / clientHistory)与 HITL 暂停恢复(pausable)——这两件事完全是
 ...
 * 不重新发起请求。
 */
export async function driveFrameStream<Frame>(
  cursor: SseFrameCursor<Frame>,
  reducer: FrameReducer<Frame>,
  ctx: AgentContext,
  ...
  for (;;) {
    ...
    const derived = reducer.add(frame);
    events.push(...derived);
...
// ───────────────────────── resumeId / captureResumeId:会话续接,服务端存历史 ─────────────────────────

/** 这轮该不该带 resume 参数:非 isNew 且已经有 id 才带,新会话一律不带。 */
export function resumeId(ctx: AgentContext): string | undefined {
 ...
 */
export function captureResumeId(ctx: AgentContext, id: string | undefined): void {
