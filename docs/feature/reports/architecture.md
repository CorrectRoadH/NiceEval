...
- **attempt 详情路由**：`view` 的 `#/attempt/@<locator>` 与 `show @<locator>`（及其证据切面）。它不是页、不占导航，**对完整结果根解析**——不随报告槽的 Scope 收窄，因此任何页里的 attempt 深链永不失效。「报告里每个数字能下钻到证据」这条不变量由它保证，不再依赖任何恒在的证据页。
...
「深链恒在」与「页共享收窄后的 Scope」因此不再冲突：Attempts 页作为普通页跟随收窄（页是对同一批数据的不同看法），被收窄滤掉的 attempt 仍可经详情路由打开。Traces 的 text 面也不再是特例——`TraceWaterfall` 的 text 面是带 `--timing` 下钻命令的 attempt 索引（[契约](library/site-components.md#tracewaterfall)），符合「索引终结于可执行命令」的省略规则。
