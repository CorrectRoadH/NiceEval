...
- `docs/adapters/README.md`：如何接被测对象、如何写 agent adapter、分档能力表(send / 事件流 / 会话 / HITL / tracing)。
- `docs/adapters/ref/agent-eval.md`：Vercel agent-eval 具体怎么写 adapter(采集/转换/落地、canonical tool name、暗号字段)——学习记录,不是 niceeval 的实现。
