...
2. **reduce(原始数据 → `StreamEvent[]`)。** 按数据到达的形状,收窄成两种官方 reducer,不用每接一个新后端就重写一个状态机:
   ...
   - OTel span **不是**第三种数据来源——曾经有过"从 span 派生事件"的设计(`otelEvents()`),已经从实现里移除,见 [OTel mixin](otel-mixin.md) 的废弃说明。现在事件流永远只从这两种官方 reducer 或手写映射来,OTel 只负责画 trace 瀑布图,和事件流完全独立(下文单独说)。
