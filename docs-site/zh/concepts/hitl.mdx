...
"挂起的现场"（比如读了一半的 SSE 流）存在本会话线的 `ctx.session.state` 里，官方件 `pausable()` 替你存取——停轮时 `hold`，回答轮 `take`。怎么和流式驱动拼起来，见[如何写好 Send](/zh/guides/write-send#问题三：中途会停下来等人吗？——决定要不要-hitl-分支)的问题三和文末骨架。
...
- [如何写好 Send](/zh/guides/write-send) — Adapter 侧实操：`pausable()` 件与流式 + HITL 的完整骨架。
