...
<ParamField body="capabilities" type="AgentCapabilities">
  ...
  <ParamField body="conversation" type="boolean">
    支持多轮会话，启用第二轮起的 `t.send()` 和 `t.newSession()`。定义里递了 `session` 件时自动为真。
  </ParamField>
...
<ResponseField name="session" type="{ state: Record<string, unknown>; id?: string; isNew: boolean }">
  当前会话线信息。`state` 是本会话线的自有状态槽（官方会话件的锚点）；`id` / `isNew` 是旧契约兼容字段。
</ResponseField>
