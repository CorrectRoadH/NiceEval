...
测试代码里的 `scriptedAgent`、`recordingSandbox`、`runnerFixture`、`attemptEvidenceFixture`、`resultsDirFixture`、`reportScopeFixture` 这类构造器统称 harness。它们是几十条测试的共同依赖：harness 的接口一动，所有用它的测试跟着动。因此 harness 是[变更预算](../README.md)最大的杠杆，本篇定义它的所有权、稳定性和设计规则。

## 所有权

- **每个 harness 归属一个 Feature**，与使用它的测试同住，由该 Feature 的测试维护者演进。Feature 清单见 [unit/README.md](README.md)。
...
- **场景要有区分力。** fixture 构造的输入必须让契约算法与常见错误算法给出不同答案；所有候选算法都会通过的输入没有证明力。各 Feature 测试文档 Fixture 规范里的区分力设计（聚合口径、现刻水位、scope 三收者）是范例。
