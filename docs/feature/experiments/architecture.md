...
ExperimentDef(纯配置数据,experiments/ 下一文件一个)
  → resolved config(调度前一次求值:合并 CLI flag / env / config 兜底,evals 与 sandbox resolver 求值)
...
- `sandbox` 是固定 `SandboxSpec` 时对全部选中 eval 复用；是 resolver 时按 `{ eval: { id, environment } }` 对每条选中 eval 求值一次。resolver 只解释 eval 的 provider-neutral environment profile，具体 provider/image/template/snapshot 仍由 experiment 决定；remote Agent 不求值。resolved spec 进入逐 eval fingerprint、provider 并发推荐值与 `ExperimentRunInfo.sandboxByEval`，resolver 函数指纹进入快照级可比性配置。
