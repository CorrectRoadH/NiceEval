...
场景已登记:`docs/engineering/testing/unit/reports.md`「Attempt 详情(view 证据室)」四行。测试方法(纯渲染、`<details>` 表达折叠态、静态 markup 断言)见 `docs/engineering/testing/unit/reports.md`「view 证据室的观察面」。
...
4. **测试**:实现 cases.md 该分区四行,一行一测,放 `src/view/app/components/` 旁(如 `attempt-detail.test.tsx`),fixture 直接构造 `ViewResult`;文件头注释按惯例标 `// cases: docs/engineering/testing/unit/reports.md`,并加 `// bug: memory/view-attempt-detail-buries-failure.md`。
