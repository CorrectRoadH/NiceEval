...
同一实验里的 eval 需要不同预制环境时，eval 只声明 provider-neutral 的 `environment` profile；experiment 把 `sandbox` 写成 resolver，再映射到 Docker image、E2B template 或 Vercel sandbox snapshot。这样任务需求留在 eval，provider 选择仍留在实验。写法见[写实验 · 让不同 eval 使用不同预制环境](/zh/how-to/write-experiment#让不同-eval-使用不同预制环境)。
