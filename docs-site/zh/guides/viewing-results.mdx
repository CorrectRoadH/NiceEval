...
└─ <run>/
   ├─ summary.json
   └─ weather-tool/            # <evalId>（含 / 时保留目录层级）
      └─ web-agent/            # <agent>
         └─ gpt-5.4/           # <model>（未指定时是 default）
            └─ a0/             # a<attempt>
               ├─ events.json
               ├─ sources.json
               ├─ trace.json
...
<Tabs>
  ...
  <Tab title="errored">eval 执行本身抛错（超时、adapter 异常等）。</Tab>
  <Tab title="skipped">eval 被主动跳过。</Tab>
</Tabs>

## 调试建议

- 先看 `summary.json` 找到失败断言，`sources.json` 把它映射回 eval 代码。
