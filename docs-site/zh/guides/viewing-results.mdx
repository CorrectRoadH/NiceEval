...
| --- | --- |
| 默认报告（总览、每组的组摘要 + 散点图 + 榜单，每行可点开看这个实验下每道题的判定与原因） | `niceeval show` |
...
总览下面，每个实验组一节（组按 experiment id 的 `/` 前缀划分，如 `compare/bub-gpt-5.4` 的组是 `compare`）：有几个组就渲染几节，都摆在同一个页面上，不用来回切换。每节从上到下是组摘要（这组题综合算下来通过率多少、覆盖几个 experiment/几道题、failed 几个、errored 几个（0 个时省略这一项）、总成本、最后一次运行时间——通过率是这组 eval 折叠计票后的 `passed / (passed + failed + errored)`，`skipped` 不进分母，和总览那个两级聚合是两码事：组摘要回答「这组题过了几道」，总览回答「整体质量几分」）、一张质量 × 成本字符坐标图（组内可比较的点少于两个时省略，画不出比较就不画）、一张榜单。榜单每行是一个 experiment，行下面跟一行紧凑摘要（覆盖几道题、几次 attempt——只在多于题数时才列出、说明有重试、最后一次运行时间）；有失败/错误题的行再往下跟一段缩进明细，每条是「哪道题、什么原因、下钻命令」。失败原因只取三处之一：`error`（环境/adapter/agent runtime 报错）、跳过原因，或未通过的 gate 断言——按题目里声明的顺序列出，多条断言用「, 」连接，断言带 detail 就是「名字: detail」，不带就只有名字；soft 断言不参与判定，从不出现在失败原因里，它的分数是报告里另一处的展示，不会混进这段文字。
