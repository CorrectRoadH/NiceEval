...
- **安装后**：只以 `node_modules/niceeval/docs-site/zh/` 里的随包文档为准。它与当前安装版本一起发布；官网或 `main` 分支可能对应另一个版本。

下文安装前出现的官网链接可以直接读取。执行完第 3 步后，先在随包目录中找到同一路由对应的 `.mdx` 文件，再继续写配置、Adapter、Experiment 或 Eval。
...
`init` 会生成 `niceeval.config.ts` 和 `evals/`。安装后的 CLI 参考在 `node_modules/niceeval/docs-site/zh/reference/cli.mdx`。

从这一步开始，停止用官网或 GitHub `main` 分支判断 API。先确认随包文档存在：

```sh
test -f node_modules/niceeval/docs-site/zh/quickstart.mdx
test -f node_modules/niceeval/docs-site/zh/reference/cli.mdx
```

然后读取：

- `node_modules/niceeval/docs-site/zh/quickstart.mdx`
- `node_modules/niceeval/docs-site/zh/reference/cli.mdx`
- `node_modules/niceeval/docs-site/zh/guides/agent-feedback-loop.mdx`
...
- 评判模型要**与被测 agent 分离**，避免同一个模型给自己打分。模型解析优先级（单次调用 → eval 级 → 全局配置）和三种评分形状见随包的 `concepts/judge.mdx`；`judge` 字段的完整定义见 `reference/define-config.mdx`。

## 第 4 步：写三件套

按第 2 步选中的方向，在 `node_modules/niceeval/docs-site/zh/` 中读取对应 `.mdx` 后，依次写：

1. **adapter**（`agents/*.ts` 或用户项目里约定的目录）——只填 `defineAgent` 的 `send`，配置走工厂参数，不写死、不读 `process.env`。先读 `concepts/adapter.mdx`，API 签名看 `reference/define-agent.mdx`，事件映射看 `reference/events.mdx`。
2. **experiment**（`experiments/*.ts`）——引用上面的 adapter，声明 `model`、`flags`、`runs` 等；被测对象是 agent 的话还要声明 `sandbox`（第 2 步已跟用户确认过的后端，默认 `dockerSandbox()`，从 `niceeval/sandbox` 导入）。**默认建一个实验组文件夹做模型对比**：`experiments/compare-models/` 下放两个文件，同一个 adapter、钉住一切、只差 `model` 各一个（比如 `compare-models/gpt-5.4.ts` 和 `compare-models/deepseek-v4-pro.ts`；`model` 是单个字符串，不接受数组）。跑一次 `niceeval exp compare-models` 就把组内各 model 并排出报告——这是 niceeval 最直观的价值演示。用哪两个模型要跟用户确认（应用支持哪些、有哪些 key）；应用接口不收模型参数的话退化成单个 experiment 文件，如实告诉用户原因。完整字段看 `guides/write-experiment.mdx`；项目级配置看 `reference/define-config.mdx`。
3. **eval**（`evals/*.eval.ts`）——**先探明这个应用是干嘛的，再写一条贴着它真实功能的 eval**：读它的 README、路由、工具定义或系统提示，找出它的核心用例（客服机器人就问一条真实的客服问题、SQL agent 就给一个真实的查询任务），拿这个用例做第一条 eval 的输入和断言，不要写"你好"这种和应用无关的占位输入。形式上仍从最小写起：一句输入，`t.succeeded()` + 一个针对预期回答的内容断言，跑通再加断言密度。`defineEval` 签名看 `reference/define-eval.mdx`；断言写法与 `t.judge` 看 `guides/authoring.mdx` 和 `guides/scoring-guide.mdx`；内置断言库看 `reference/expect.mdx`。

以上相对路径都位于 `node_modules/niceeval/docs-site/zh/`。参数怎么从 experiment 流到 adapter、静态配置和每轮动态值怎么分（工厂参数 vs `ctx`），完整讲解见 `guides/connect-your-agent.mdx`。
...
先读 `node_modules/niceeval/docs-site/zh/guides/agent-feedback-loop.mdx`，让 AI 用 `niceeval show`、`--transcript`、`--trace` 和 `--diff` 完成运行、观察、修改和重跑。查看器说明在 `node_modules/niceeval/docs-site/zh/guides/viewing-results.mdx`。没跑通按报错位置分三类排查（详细排查表在 `node_modules/niceeval/docs-site/zh/guides/connect-your-agent.mdx`）：`fetch` 直接抛错 → 应用没起来或 URL 不对；`t.succeeded()` 不过 → 应用回了非成功状态；只有内容断言不过 → 接入已经通了，调断言或调应用。
...
| 能做什么 | 改动量 | 好处 | 文档 |
|---|---|---|---|
| 工具调用断言（`t.calledTool()` 等） | 只改 adapter：把应用响应映射成标准事件流，约 10–30 行映射代码 | eval 能断言"agent 有没有调对工具、参数对不对"，不再只看最终回复 | `guides/write-send.mdx`、`reference/events.mdx` |
| 多轮对话、会话隔离 | 只改 adapter：接上 `ctx.session`（`history()` 或 `id` + `capture()`），几行到十几行 | eval 能写多轮场景、`t.newSession()` 验证会话间不串味 | `guides/write-send.mdx` |
| 人工审批流（HITL） | 只改 adapter：停轮返回 `waiting` + `input.requested`，回答轮续跑，约 10–20 行 | eval 能覆盖"批准/拒绝之后 agent 行为对不对"这类审批场景 | `concepts/hitl.mdx` |
| 调用瀑布图（升 Tier 2） | 应用已有 OTel 埋点（第 2 步探过）：只是把 span 多发一份给 niceeval，几行配置；没埋点：补一段通用 OTel 初始化 | `niceeval view` 里看到应用内部每次模型调用、工具执行的耗时和 token 时间线；不影响任何断言 | `guides/connect-otel.mdx` |
| feature A/B 对比（升 Tier 3） | 改应用：把变体暴露成 `flags` 可切换的配置，改动量取决于应用；已有 A/B 开关（第 2 步探过）就是现成入口 | experiment 层面直接对比"改 prompt / 换工具集 / 开关 feature 谁更好" | `concepts/tier.mdx`、`concepts/experiment.mdx` |

这些路径同样相对于 `node_modules/niceeval/docs-site/zh/`。共同点也要讲给用户：这些全是给 adapter 或应用加增量，**已写的 eval 一行不用改**。三档投入分别买到什么、什么时候值得升级，见 `concepts/tier.mdx`。第 2 步探到应用已有 OTel 埋点的话，瀑布图那条要主动推荐——成本接近零。
