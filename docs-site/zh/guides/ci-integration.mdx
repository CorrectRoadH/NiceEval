...
niceeval: errored locator=@2h8m4k1 eval=fixtures/button experiment=ci/codex phase=sandbox-provision code=sandbox-rate-limit message="E2B sandbox allocation failed after 5 attempts"
...
退出码是第一层红绿信号；JSON、JUnit 和结果快照是完整机器接口，日志行只用于搜索和 annotation。`errored` 行带 locator、正式 phase、稳定 code 和一层 message。详细 cause、stack 和 diagnostics 保存在 Attempt 的 `result.json`，可在保留 artifact 后运行 `niceeval show @<locator>` 回顾。
