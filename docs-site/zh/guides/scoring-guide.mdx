...
| 目标 | 推荐机制 |
|---|---|
| 回复包含固定内容 / 命中正则 | `includes` |
| 结构化输出符合 schema | `matches` |
...
t.check(t.reply, includes(/order #\d+/));      // includes 也接受正则
t.check(turn.data, equals({ intent: "refund" }));
t.check(turn.data, matches(orderSchema));      // matches 是 schema 校验（zod 等）
```
