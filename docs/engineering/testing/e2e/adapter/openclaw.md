...
仓库 ID `openclaw`，group `sandbox`，`e2e.json.requires.docker: true`。被测对象是 `openClawAgent()`
在 Docker Sandbox 里的完整生命周期（契约见
[OpenClaw 契约页](../../../../feature/adapters/sdk/openclaw/README.md)）。
...
- **OTel**：适配器复用 canonical OTel
  mapper，执行树的时间注释就是记录成立的展示证明。OTel 内容关闭时 trace 缺失只影响 trace 本身——执行树节点照常出现、只是显示 timing
  unavailable，事件流断言全部照常通过；这条降级路径本身就是一条验收断言。
