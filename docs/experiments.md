...
- **`fasteval.config.ts`(`defineConfig`)** = 项目级默认:`judge`、`reporters`、并发 / 超时,以及项目级的[生命周期钩子](lifecycle.md)(`hooks.run` / `hooks.sandbox`,实验的同名钩子叠加在其上)。**沙箱后端不在 config** —— 由 experiment 的 `sandbox` 决定。
