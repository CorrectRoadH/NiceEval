...
import {
  ...
  MetricScatter, ExperimentTable, Scoreboard,
  costUSD, passRate,
} from "niceeval/report";

export default defineReport(async ({ selection }) => (
  <Col>
    {/* 想把官方默认那两块摆在自己口径旁边当对照，就照它的写法自己摆同样两块 */}
...
报告文件里摆组件有两种写法。散点图 `MetricScatter` 和实验明细表 `ExperimentTable` 直接收 `selection`：`<MetricScatter selection={selection} … />`、`<ExperimentTable selection={selection} />`，宿主在渲染前替你把数据算好，报告里不用写 `await`、`.data()` 或中间变量——默认报告就是这么写的。其余组件（`Scoreboard`、`MetricTable`、`GroupSummary` 等）收算好的 `data`：你在报告函数体里 `await` 对应的 `.data()` 计算函数，把数据折出来再传进去（`<Scoreboard data={await Scoreboard.data(selection, …)} />`）。`MetricScatter` / `ExperimentTable` 收 `selection` 的写法也能改收 `data`——把某块嵌进你自己已经跑起来的 React 应用，或数据是预先算好、缓存、跨进程读来的时候用；这时组件不碰结果目录，只渲染你给的数据。同时给它们传 `data` 和 `selection`、或两者都不传，类型检查都会报错。
...
默认报告没有特权：它就是上面导出的那个值 `CostPassRateComparison`，你的报告和它同级。要「官方摆法 + 自己的口径」，把它那两块组件（`MetricScatter` + `ExperimentTable`）照原样摆进你的树，和你自己的口径并排对照——默认报告没有任何私有零件，你摆出来的和它一模一样；不要官方那两块就不摆。组合发生在你的 tsx 里，没有 tab、没有注册表、没有配置。
...
`GroupSummary` 之后想接着摆散点图或实验明细表，把同一份 `scoped` Selection 直接传给 `<MetricScatter selection={scoped} points="experiment" series="agent" x={costUSD} y={passRate} />` 和 `<ExperimentTable selection={scoped} />` 就行——组件替你从这份收窄好的 Selection 折出数据。想自由换行维度和指标列，再用下面的通用 `MetricTable`。

## 换口径：自定义指标

默认报告的实验明细表就是 `ExperimentTable`——它固定保留配置、KPI、逐 Eval/Attempt 和 raw sample。下面的例子是另一种需求：用通用 `MetricTable` 自由换指标口径——每个组件的数据算法都挂在自己身上，换口径只需换 columns：
