...
Every [NiceEval](https://niceeval.com/) eval is a TypeScript file that default-exports `defineEval(...)`. The core rules are simple: **path is identity**, **one file is one eval**, and **write linearly with assertions next to the interaction they check**.
...
## Report long steps and diagnostics from an eval

`setup` prepares the task fixture for this eval. Its second argument is bound to the eval setup phase; feedback sent from inside `test(t)` is bound to the eval run phase:
