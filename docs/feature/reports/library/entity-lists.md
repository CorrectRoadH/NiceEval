...
| 列 | 内容 |
|---|---|
| Experiment | experiment id；副行以“`8 evals` / `8 个 Eval`”显示 eval 数，attempt 数多于 eval 数时再显示 attempt 数，最后跟最后运行时间；不把 Eval 翻成“题”。传 `relativeTo` 时行标签去掉与该父路径相同的前缀，只显示 id 末段（与同组散点的点标签同源）；完整 id 仍用于排序键、过滤和折叠展开 |
...
`relativeTo` 是可选的父路径：设置后每行只显示相对该路径的 id 末段，避免在已经以组为标题的上下文里重复文件夹名。这不是独立的短标签字段，也不从 id 猜别名：去掉组前缀后的组内 id 仍然完整显示。默认 `ExperimentComparison` 给每组的 `ExperimentList` 传入组键（experiment id 的父目录），因此 web 与 text 两面组内各行显示末段而非完整 `组/末段`，与同组散点的点标签保持一致；根目录单例组的 id 不含该前缀，照原样显示完整 id。独立使用 `ExperimentList` 且不传 `relativeTo` 时始终显示完整 id。无论显示形态如何，排序键、过滤匹配和折叠展开都用完整 id，不受影响；视觉系列色跟随 Agent，不用每个 experiment id 生成一种与散点系列无关的颜色。
