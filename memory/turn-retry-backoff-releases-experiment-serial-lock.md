...
设计裁决见 [experiment-gate-tenure-ruling](experiment-gate-tenure-ruling.md)(两级闸按持有期分工),实现 TODO 在 `plan/experiment-gate-full-attempt-tenure.md`。要点:

`src/runner/run.ts` 的 turn 级 `ConcurrencySlot` 只释放/收回 globalSem,退避期间**继续持有 runSem**:
...
- 同步义务:`docs/feature/error-classification/architecture.md`「退避与槽位」行补一句「实验级闸不释放」;run.ts L746-761 的注释重写。
- 回归校验(下游已给):mempal 型实验全程 `running` 恒 =1;各 attempt 记忆回存 `savedAt` 无重叠;记忆 KB 单调不回退。单测方向:fake adapter 第一次 send 抛可重试错误制造退避窗口,断言退避期间同 run 的第二个 attempt 不进入 body(现有 send-retry 单测注入 `sleep`/`random` 的写法可复用)。

注意 `src/sandbox/retry.ts` 的 `ProvisionSlot`(sandbox provisioning 退避)只释放 sandboxSem,不涉 runSem,无此问题,不要顺手改。
