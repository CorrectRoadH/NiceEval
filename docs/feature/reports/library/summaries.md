...
两级计票与 `endToEndPassRate` 互不反推：成功率来自官方两级指标引擎，渲染面不得从任一计票现场重算。Scope warning 不进 `ScopeSummaryData`：警告的呈现件是 [`ScopeWarnings`](site-components.md#scopewarnings)，摘要数据不复制它的输入，同一份事实不在页面上出现两次。
