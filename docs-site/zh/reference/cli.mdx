---
title: "NiceEval CLI：命令、flags 和退出码参考"
sidebarTitle: "命令行"
description: "NiceEval CLI 参考：exp、show、view、init、list 和 clean 命令，以及结果下钻、experiment、eval 过滤、并发、预算和 CI 输出。"
---

[NiceEval](https://niceeval.com/) CLI 是发现、运行和查看 eval 的入口。实际执行 eval 时采用 experiment-first 模型：**`exp` 选择可签入的运行配置**，experiment 后的位置参数才按 eval ID 前缀过滤。agent、model 和 flags 写在 `experiments/`，不靠临时 CLI 参数。

## 命令

<CardGroup cols={2}>
  <Card title="npx niceeval exp &lt;group&gt; [id-prefix]" icon="flask">
    运行命名实验组或配置。可选尾随位置参数按 eval ID 前缀过滤。
  </Card>
  <Card title="npx niceeval init" icon="wand-magic-sparkles">
    创建空的 `evals/` 目录和最小的 `niceeval.config.ts`；不生成示例 eval 文件。同时写入/刷新 niceeval 指引区块——项目已有 `AGENTS.md` 就写那份，只有 `CLAUDE.md` 就写进 `CLAUDE.md`，都没有则新建 `AGENTS.md`（见[Agent 反馈闭环](/zh/guides/agent-feedback-loop)）。
  </Card>
  <Card title="npx niceeval list" icon="list">
    发现并打印所有 eval，不运行。
  </Card>
  <Card title="npx niceeval clean" icon="trash">
    删除 `.niceeval/` 下历史快照 artifacts。
  </Card>
  <Card title="npx niceeval show [id-prefix]" icon="terminal">
    在终端读取结果；按 eval ID 前缀下钻断言、transcript、trace、diff 和历史。
  </Card>
  <Card title="npx niceeval view" icon="eye">
    打开网页结果查看器，读默认报告并交互浏览 transcript、trace、diff 与历史；默认范围与 `show` 相同。
  </Card>
</CardGroup>

## 输出语言

[NiceEval](https://niceeval.com/) 的 CLI 和运行时文案支持本地化，不需要在 `niceeval.config.ts` 里加配置。需要固定输出语言时用环境变量：

```bash
NICEEVAL_LANG=en npx niceeval list
NICEEVAL_LANG=zh-CN npx niceeval list
```

检测顺序是 `NICEEVAL_LANG`、`NICEEVAL_LOCALE`、`LC_ALL`、`LC_MESSAGES`、`LANG`。以 `zh` 开头的值使用 `zh-CN`,其它语言使用 `en`;都没有时默认 `zh-CN`。这只影响终端/runtime 文案,不改变结果 JSON 里的机器字段,也不翻译 LLM judge prompt。

## `npx niceeval exp [group|config] [id-prefix...]`

```bash
# 运行 experiments/ 下全部实验
npx niceeval exp

# 运行一个实验组
npx niceeval exp compare-models

# 只运行该组里的一个配置(id = 组/文件名,不带扩展名)
npx niceeval exp compare-models/gpt-5.4

# 在该组里只运行 ID 以 weather 开头的 eval
npx niceeval exp compare-models weather
```

<Note>
  eval 过滤参数只出现在 experiment 选择之后。裸 `npx niceeval weather` 不会运行；请使用 `npx niceeval exp local weather` 或 `npx niceeval exp compare weather`。
</Note>

第一个位置参数既能填实验组名，也能填单个配置的完整 id：精确匹配到 `组/文件名` 时只跑组内那一个配置，适合逐配置验证或对比排查某一格结果，不用把其它配置文件挪出目录。

## 常用 flags

下表由 CLI 的 flag 解析表生成，表外的 flag 一律按未知 flag 报错并以非零状态退出。**没有**用于选择 sandbox 后端的 CLI flag：sandbox 后端只能写在代码里——在 experiment（`defineExperiment`）里设置 `sandbox`，或者在 `niceeval.config.ts`（`defineConfig`）里设置作为项目级兜底，值来自 `niceeval/sandbox` 的 `dockerSandbox()` / `vercelSandbox()` / `e2bSandbox()`。

{/* GENERATED:BEGIN cli-flags */}

{/* 本区块由 pnpm docs:reference 从源码注释生成,勿手改;要改文案,改对应源码的 TSDoc/JSDoc(映射见 scripts/generate-reference.ts) */}

| Flag | 类型 | 说明 |
|---|---|---|
| `--agent` | string | experiment 运行不支持该 flag。要换 agent,请在 `experiments/` 下新增或复制一个配置文件。 |
| `--model` | string | experiment 运行不支持该 flag。要换模型,请新增或复制一个 experiment 文件并修改 `model`。 |
| `--runs` | number | 每个 eval 运行多少次,常用于 pass@N。 |
| `--max-concurrency` | number | 设置同时运行的 eval 数量。 |
| `--timeout` | number | 单个 attempt 的超时时间,单位毫秒。 |
| `--budget` | number | 整次运行的预算上限(美元)。 |
| `--tag` | string | 只运行带有该 tag 的 eval(见 `defineEval` 的 `tags`)。 |
| `--junit` | string | 额外写一份 JUnit XML 报告到指定路径,供 CI 消费。 |
| `--json` | string | 额外写一份 JSON 结果(`RunSummary` 原样序列化)到指定路径,供 CI 或下游脚本消费。 |
| `--out` | string | `view` 命令专用:把结果查看器静态导出到指定目录。 |
| `--port` | number | `view` 命令专用:指定本地服务器监听端口。 |
| `--transcript` | boolean | `show` 命令专用:渲染单个 eval 的完整对话与工具调用(证据切面)。 |
| `--trace` | boolean | `show` 命令专用:渲染单个 eval 的 trace 瀑布文本版(证据切面)。 |
| `--diff` | boolean | `show` 命令专用:sandbox 里的文件改动摘要;`--diff=<文件路径>` 看单个文件的完整改动(路径必须 `=` 连写)。 |
| `--history` | boolean | `show` 命令专用:跨 run 时间轴,只列真实执行;与 `--report` 互斥。 |
| `--experiment` | string | `show` / `view` 命令专用:Selection 只留该实验。 |
| `--attempt` | number | `show` 命令专用:指定详情 / 证据切面看第几次 attempt(与展示一致的 1 计序号)。 |
| `--run` | string | `show` / `view` 命令专用:钉死看某一个结果目录(某次快照或 `copySnapshots` 产物)。 |
| `--report` | string | `show` / `view` 命令专用:用你的报告文件替换默认报告(文件默认导出 `defineReport(...)`)。 |
| `--dry` | boolean | 只打印本次会匹配到的 eval × 运行配置,不实际执行。 |
| `--quiet` | boolean | 关闭控制台 / live 的逐条结果与末尾汇总(attempt 进度行仍写 stderr);errored / failed 的结果各在 stderr 补一行摘要,passed / skipped 静默;reporter 仍会写 artifacts。 |
| `--force` | boolean | 忽略上次运行结果,不跳过已通过的 (experiment, eval) 组合,强制全部重跑。 |
| `--strict` | boolean | CI 中推荐使用:让软阈值(`soft`)失败也计入整条 eval 的 verdict。 |
| `--early-exit` / `--no-early-exit` | boolean | 某个 eval 的一次 attempt 通过后,停止该 eval 剩余的 attempts。 |
| `--open` / `--no-open` | boolean | `view` 命令专用:启动后自动打开浏览器(默认行为)。 |
| `--help` | boolean | 打印用法说明并退出。 |
| `--version` | boolean | 打印 niceeval 的版本号并退出。 |

{/* GENERATED:END cli-flags */}

## `list`

```bash
npx niceeval list
```

用于检查 eval 发现、ID 和配置加载是否正常。

## `exp`

```bash
npx niceeval exp compare-models
npx niceeval exp compare-models weather-tool
```

运行命名 experiment，用矩阵比较 agents、models 或 flags。第二个参数开始是 eval ID 前缀过滤。

## `view`

```bash
npx niceeval view
```

打开本地结果查看器。它和 `show` 共用同一份默认报告和同一套默认选择——对每个 experiment、每个 eval，取历次运行里最新的那份判定；只补跑部分 eval 时，其余 eval 从更早的运行补齐。`show` 输出终端文本，`view` 输出网页并提供可交互的证据浏览；eval ID 前缀、`--experiment`、`--run` 对两者的收窄一致。完整说明见[查看结果](/zh/guides/viewing-results)。

## `show [id-prefix...]`

```bash
npx niceeval show
npx niceeval show weather/brooklyn
npx niceeval show weather/brooklyn --transcript
npx niceeval show weather/brooklyn --trace
npx niceeval show fixtures/button --diff
npx niceeval show weather/brooklyn --history
```

`show` 是终端结果入口，适合人直接阅读，也适合 coding agent 在上下文窗口里逐级下钻。位置参数只按 eval ID 前缀筛选；不带位置参数时显示默认报告，指定到单个 eval 时显示各 experiment 的 attempt、断言、错误与用量。

证据切面 `--transcript`、`--trace`、`--diff` 要求最终只匹配一个 eval。多 experiment 或多 attempt 时，默认选择最新一次失败的 attempt；用 `--experiment` 和 `--attempt` 指定其它结果。`--run` 钉住某次结果目录，`--history` 查看跨 run 趋势。完整的阅读顺序、输出示例和 artifact 说明见[查看结果](/zh/guides/viewing-results)。

## `--early-exit` 与 `--strict`

`--early-exit` 默认就是 `true`：某个 eval 的一次 attempt 通过后，会自动停止该 eval 剩余的 attempts（省钱）。所以显式传 `--early-exit` 没有效果，真正有用的是反过来关掉它——`--no-early-exit`，让 `--runs` > 1 时把每次 attempt 都跑完（测真实通过率而非提前收尾）。

`--strict` 不是"更严格地报错"，而是改变软阈值断言的判定：`.atLeast(n)` 这类软阈值（`soft` severity）平时失败不会把整条 eval 判为 `failed`（只是记一条不达标的断言），加了 `--strict` 之后，软阈值没达标也会让整条 eval 的 verdict 计为 `failed`。CI 中推荐加上，避免"断言分数不够但 eval 显示通过"的情况被放过。

## 环境变量覆盖

以下环境变量对应部分 flag，用于不便传 CLI 参数的场景（如 CI 环境配置）：

| 环境变量 | 对应 flag |
|---|---|
| `NICEEVAL_RUNS` | `--runs` |
| `NICEEVAL_MAX_CONCURRENCY` | `--max-concurrency` |
| `NICEEVAL_TIMEOUT` | `--timeout` |
| `NICEEVAL_BUDGET` | `--budget` |

优先级从高到低：CLI flag > 环境变量 > experiment / `niceeval.config.ts` 里的值 > 内置默认值。四个环境变量都要求合法数字，解析失败会直接报错退出。

[NiceEval](https://niceeval.com/) 还会在启动时自动加载 cwd 下的 `.env` 文件（不需要额外配置，也不覆盖已经存在的环境变量），常用于本地放 `ANTHROPIC_API_KEY` 这类鉴权信息，不用每次手动 `export`。

## 退出码

| 退出码 | 含义 |
|---|---|
| `0` | 全部 eval 通过（或 `list`、`init`、`clean` 等命令正常完成）。 |
| `1` | 存在 `failed` / `errored` 的 eval；或命令用法错误（如 `exp` 没匹配到任何实验、传了不支持的 flag 组合）。 |
| `2` | 运行时崩溃（未捕获异常），CLI 顶层兜底捕获后退出。 |
| `130` | 收到中断信号（Ctrl-C / SIGTERM）后退出。 |

需要 CI 把软阈值失败也当失败处理时，同时使用 `--strict`（见上）。
