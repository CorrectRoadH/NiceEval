...
![Full round trip of one t.send: the eval calls t.send, the runner assembles TurnInput and ctx, the adapter calls your app and returns a Turn with the standard event stream.](/images/agent-turn-roundtrip-en.svg)

## Starting point: what shape is your app's interface?

[NiceEval](https://niceeval.com/) does not invent protocols. Your app is almost certainly built on one of the industry protocols below (or a variant of one), and the official pieces cover exactly these shapes:
...
**This step unlocks**: `t.reply`, `t.messageIncludes()`, all the conversation material for judges, and **model comparison** on the experiment side (`ctx.model` is exactly the `model` declared in the experiment; the runner hands it to you verbatim and the Adapter does not interpret it, only forwards it — you do not need to wait for a later step, and the app does not need to cooperate with any change; see [Tier](/concepts/tier) for the tiers).
