...
`codex exec` 一律以 `--json --dangerously-bypass-approvals-and-sandbox --skip-git-repo-check --dangerously-bypass-hook-trust` 运行，首轮与 `codex exec resume` 续轮相同。hook 信任 bypass 与审批 bypass 属同一信任层级：沙箱运行是 headless 的，codex 对非 managed 来源 hook 的交互式信任确认永远无人应答，不 bypass 时这些 hook 被静默跳过且零报错——插件依赖的注入/捕获行为整体失效而无任何征兆。沙箱里的每个 hook 来源都由实验配置显式声明（`plugins`、`postSetup`、`configFile`），声明即审计，因此不设开关。`bypass_hook_trust` 是 runtime-only 参数，`config.toml` 写不了，只能进 exec 命令行。
