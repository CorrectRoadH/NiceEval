...
- **Tier 2:侵入**。要评「改 prompt / 换工具集 / 开关某个 feature 谁更好」这类 **feature A/B test**,对照点在应用内部,无侵入拿不到——需要变更内部代码,把变体暴露成外部可选的配置(经 experiment 的 `flags` → `ctx.flags` 透传),或让 adapter 进程内直调内部函数。
