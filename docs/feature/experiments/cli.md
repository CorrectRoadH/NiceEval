...
面板一律圆角框、标题嵌在上边框左侧、可选的一个指标嵌在下边框或上边框右侧,内部再分节时用 `├─ 小节 ─┤` 横隔。哪些是面板固定为:计划(`PLAN`)、运行中的 live 面板、结束结论(`PASSED` / `FAILED`)、`FAILURES`、`NEXT`(含 `RESULTS` 小节)。

失败行、错误摘要、诊断和运行级瞬时通知是流事件,以无框行追加进 scrollback——它们逐条到达、条数不可预知,逐条画框会把 scrollback 变成框的堆叠,而且每条失败最终都会在 `FAILURES` 面板里被再收拢一次。

框只是 TTY 呈现层,不携带任何契约信息:

- 框宽跟随终端宽度,上限 100 列;终端窄于 60 列或不是 TTY 时整套框线降级为无框纯文本,内容与分节标题不变。
- 边框吃掉 2 列内容宽和 2 行内容高,可见项数按**内高**计算,不按终端高度。
- 上下边框先保证嵌入的标题和指标完整:横线可以缩到最少 1 段;缩到头仍放不下时先截断标题(命令过长时保留 `niceeval exp` 和末尾参数、中间补 `…`),再放弃右侧指标——指标另有归宿(elapsed 与成本在结束面板里重复,失败总数在 `FAILURES` 正文里重复),标题没有。
- 行宽按显示列而不是码点数计算:renderer 用宽度表量测,并把 East-Asian-Ambiguous 一律当作 1 列,框内内容也只用按 1 列渲染的符号(`·` `●` `…` `×` `✗`)。这两件事必须一致,否则同一份内容在 ambiguous=wide 的 locale 下会把每一条右边框顶歪。
- 终端 resize 触发 live 面板整体重绘;这不是状态变化,不放宽刷新节奏。
- 框字符只用 `╭╮╰╯│─├┤`,不用双线、粗线或阴影;色彩只标结论(`PASSED` 绿 / `FAILED` 红),不给边框上色。
