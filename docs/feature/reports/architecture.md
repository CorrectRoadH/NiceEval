...
分区发生在 Scope 已完成范围收窄和现刻水位选择之后、任何指标计算之前。每个组独立调用 `scopeSummaryData`、`metricScatterData` 与 `experimentListData`；所以组外 attempt 不可能污染该组的坐标尺度、series 连线、通过率、成本、排序或缺数据计数。Scope warning 仍属于整份选择，由页首的 `ScopeWarnings` 统一显示，不复制进每组。
...
| 默认填充 | [内建报告](library/built-in.md)首页（报告页）：`ExperimentComparison` 多组时只输出组索引与单组查看命令，单组时输出该组独立的成本 × 端到端通过率散点与 `ExperimentList`；尾部附 Attempts / Traces 页索引 | 同一内建报告：初始页是报告页，导航含三页；`ExperimentComparison` 完整组索引 + 当前组独立的散点与可排序、可过滤 `ExperimentList` 固定列表格；切组不重新读取 Scope |
