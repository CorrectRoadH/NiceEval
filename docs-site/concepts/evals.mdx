...
This keeps IDs stable and readable, and naturally consistent with the directory structure.
...
`gate` is a hard threshold. Failure makes the eval fail. `soft` contributes a score without necessarily failing the eval. See [Assert](/concepts/assert) for the full rules.
...
- [Experiment](/concepts/experiment) - The other half: who to evaluate and how to run it, and why it is kept separate from the eval (late binding).
- [Assert](/concepts/assert) - The full verdict rules for gate and soft assertions.
