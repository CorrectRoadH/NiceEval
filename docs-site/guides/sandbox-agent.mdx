...
export default defineSandboxAgent({
  ...
  async send(input, ctx) {
    ...
    if (ctx.session.id) args.push("--resume", ctx.session.id);   // undefined on a fresh session line
    ...
    ctx.session.capture(shared.sessionIdFromClaudeTranscript(raw));  // enables multi-turn resume
