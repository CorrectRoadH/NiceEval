...
这一节是全仓终端框线的单一契约。`show` 的证据区块、自定义报告的 text 面、`exp` 的 [live 面板与结束面板](../../experiments/cli.md#框线体裁)都按它渲染，别处只引用不复述。
...
**圆角框标区域，直角框标数据格。** `Grid` 的 `variant="boxed"` 给每个 cell 的 `┌─┐` 是直角（见下），与区域框的圆角区分开：一眼能看出哪一层是结构、哪一层是数据。
