...
description: "用 defineEval 编写 NiceEval Eval，包括单轮断言、多轮对话、数据驱动测试、sandbox workspace 和 setup / teardown 任务夹具。"
---
...
`setup(sandbox, ctx)` 准备 fixture；配一个 `teardown(sandbox, ctx)` 收尾，两者合起来是这条 eval 的任务夹具，每个 Attempt 各执行一次。执行顺序：`setup` 在沙箱环境钩子和 git 基线锚点之后、`test(t)` 之前跑；`teardown` 是 Attempt 收尾链的第一段（先 eval teardown，再 agent teardown，最后 sandbox teardown），这时沙箱还活着，收尾代码可以照常读沙箱。

大多数 fixture 不需要 `teardown`——写进沙箱的起始文件、装的依赖随沙箱销毁自动没了。需要 `teardown` 的是**沙箱外**的夹具：在共享外部服务里为这个 Attempt 建的临时资源（临时 repo、bucket、队列 topic），不收就泄漏。
