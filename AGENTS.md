...
- 写、改或评审测试：先读 [`docs/engineering/testing/README.md`](docs/engineering/testing/README.md)（两层体系、变更预算、改了什么跑什么）；写单元测试前再读 [`docs/engineering/unit-tests/README.md`](docs/engineering/unit-tests/README.md) 与对应 Feature 的 `docs/engineering/unit-tests/<feature>/cases.md` 登记表；造或改 fixture 前读 [`docs/engineering/unit-tests/harness.md`](docs/engineering/unit-tests/harness.md) 与该 Feature 的 `unit-tests/<feature>/README.md` 测试架构页
...
- 测试求质不求量：先登记后写测——测试只实现对应 `cases.md` 已登记的场景行，新场景先补行再动手（[`docs/engineering/unit-tests/registry.md`](docs/engineering/unit-tests/registry.md)）；答不出「证明哪条契约、删了会放走哪类错误」的测试不写，同一场景的第二条测试是维护负担。
