...
它既能评估 Claude Code / Codex 写的 coding-agent 插件、Hook 和 Skill，也能直接评你自己的 AI agent 应用。无论你的 agent 基于 AI SDK、LangGraph、Pi 还是自研 loop，都可以通过 Adapter 接进同一套 eval surface。
