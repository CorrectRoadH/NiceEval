...
| 列 | 内容 |
|---|---|
| Experiment | experiment id；副行以“`8 evals` / `8 个 Eval`”显示 eval 数，attempt 数多于 eval 数时再显示 attempt 数，最后跟最后运行时间；不把 Eval 翻成“题”。传 `relativeTo` 时行标签去掉该路径前缀；完整 id 仍用于排序键、过滤和折叠展开 |
...
`relativeTo` 是报告作者显式提供的可选路径前缀；设置后只缩短显示，不改变身份。`ExperimentComparison` 原样透传自己的 `relativeTo` prop 到这里，缺省不设，因此不显式指定时仍显示完整 experiment id；两者都不从 Scope 里的 experiment id 反猜前缀。排序键、过滤匹配和折叠展开也都用完整 id；视觉系列色跟随 Agent。
