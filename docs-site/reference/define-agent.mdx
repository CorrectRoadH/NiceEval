...
Use this for coding agent CLIs. The produced `Agent.kind` is always `"sandbox"` — filesystem assertions like `t.sandbox` and `t.fileChanged()` unlock only on this kind of agent. `ctx.sandbox` is the handle to the current isolated environment:
