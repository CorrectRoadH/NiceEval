...
<Tabs>
  ...
  <Tab title="Vercel">
    ...
    **Requirements:** set `VERCEL_TOKEN` or `VERCEL_OIDC_TOKEN` in your environment.
    ...
    ```shell
    VERCEL_TOKEN=... npx fasteval exp local fixtures/button --sandbox vercel
    ```
  ...
  <Tab title="Third-party">
    ...
    ```shell
    npx fasteval exp local fixtures/button --sandbox e2b
    ```
  </Tab>
</Tabs>
...
<Steps>
  <Step title="CLI flag (highest priority)">
    ```shell
    npx fasteval exp local fixtures/button --sandbox docker
    npx fasteval exp local fixtures/button --sandbox vercel
    ```
  </Step>
