...
- 被测对象是其它 agent 系统（HTTP / gRPC 服务）：无侵入接入，优先[OTel 接入](/zh/guides/connect-otel)，接不了 OTel 就手写事件映射——[`examples/zh/tier1`](https://github.com/CorrectRoadH/niceeval/tree/main/examples/zh/tier1) 下五个示例全是这条路。
...
- [OTel 接入](/zh/guides/connect-otel) — 已埋点应用的免映射接法。
