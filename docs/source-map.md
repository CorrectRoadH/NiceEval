...
- **`Usage` 落盘形状与 `src/o11y/types.ts` 不一致**:`docs/feature/results/architecture.md#usage` 的目标 `Usage` 是 `inputTokens?`/`outputTokens?`(均可选,协议不提供就省略)、`cacheReadTokens?`、`cacheCreationTokens?`、`reasoningTokens?`、`requests?`,不含 `costUSD`(成本是消费端估算,不落在 `Usage` 里)。当前 `src/o11y/types.ts` 的 `Usage` 是 `inputTokens: number`/`outputTokens: number` 必填、字段名是 `cacheWriteTokens` 不是 `cacheCreationTokens`、没有 `reasoningTokens`、多出 `costUSD?: number`。这套旧形状波及 `src/context/session.ts`、`src/runner/{attempt,types}.ts`、`src/show/{render,compose}.ts`、`src/agents/{langgraph,sdk-streams,ai-sdk}.ts`、`src/runner/reporters/braintrust.ts`、`src/o11y/{types,cost}.ts`、`src/o11y/parsers/{bub,openclaw}.ts`、`src/scoring/scoped.ts`、`src/report/{index,model/{metrics,types}}.ts`、`src/report/components/{shared,fixtures}.ts`、`src/report/components/attempt-detail/{compute,faces,index,AttemptSummary,UsageTable}.tsx?`、`src/report/components/entity-lists/{compute,index,AttemptList,ExperimentList,faces}.tsx?`、`src/report/components/summaries/{compute,index}.tsx` 共 30 余个非测试文件。修复见实施计划 `plan/show-scope-slice-json.md` 节点 A1(「`Usage` 形状对齐 `src/types.ts`」)。
