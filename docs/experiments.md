...
| `setup` | — | **删** | 环境预置不进 experiment 本身:按实验变化的环境挂在 `sandbox` 字段的 `SandboxSpec.setup()` / `.teardown()`,任务夹具写 `EvalDef.setup` / `test()`,连 agent 写 `SandboxAgent.setup`,整轮共享服务用外部编排(见 [环境预置放哪](sandbox.md#环境预置放哪)) |
