...
- `docs/engineering/unit-tests/experiments-runner/cases.md`
...
- `docs/engineering/unit-tests/results/cases.md`
...
仓库直接在 `main` 上多人共享。实现前先看 `git status`；只提交本计划涉及的显式路径，不要把 `docs/engineering/e2e-ci/**`、`test/e2e-structure.test.ts`、`vitest.config.ts` 等现有并行改动夹带进 commit。
