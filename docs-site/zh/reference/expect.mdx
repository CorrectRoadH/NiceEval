...
```ts
matches(schema: unknown): Assertion
```

用 schema 校验值——支持 Standard Schema（zod 等）或 zod 风格的 `.safeParse` / `.parse`。要匹配正则，用 `includes(/正则/)`。

```ts
t.check(turn.data, matches(z.object({ orderId: z.string() })));
```
...
- `gate(threshold?)`：硬门槛，失败会让 eval 失败。
- `atLeast(threshold)`：软阈值，参与评分，默认不让 eval 失败（`--strict` 下才会）。
...
const validEmail = makeAssertion({
  name: "valid email",
  score: (value) => (typeof value === "string" && value.includes("@") ? 1 : 0),
});
