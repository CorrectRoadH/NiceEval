...
**OTel 不是这三段之外的第四条路线,是 reduce 段的第三种数据来源,而且只影响这一段。** 它不能让你跳过"编排"这一段:不管事件是从帧流直接解析出来,还是从 span 派生出来,会话续接和 HITL 暂停恢复永远得单独实现——span 没有"等人输入"语义,也不知道"这轮该不该带历史"。OTel 是**纯观测**:它决定事件流(`Turn.events`)怎么产出,不改变 `send` 的控制流,更不参与会话续接或 HITL 判定。声明 `events: otelEvents()` 之后,`send` 该怎么读流、怎么处理审批帧、怎么续接会话,和不声明时完全一样——mixin 唯一做的事是"把这一轮收到的 span 转换成 `StreamEvent[]`,和 `send` 自己返回的 events 合并"。它真正省的只是 reduce 段里"文本/工具"这一小块,而且要看应用有没有埋点;它不可替代的价值是时间轨(`niceeval view` 的瀑布图),这个只有它能给,和行为断言完全正交。详见 [OTel mixin](otel-mixin.md) 与 [OTel 接入指南 · 边界](../../docs-site/zh/guides/connect-otel.mdx)。
