...
<Warning>
作用域断言受 agent 的能力位守卫。agent 没声明 `toolObservability: true` 时调用 `t.calledTool()` 会在运行时被能力守卫拦下、报清晰错误（tsx 直跑不做类型检查，守卫在运行时兜底），而不是对着不完整的事件流假通过。
</Warning>
...
- [写 send](/zh/concepts/write-send) — 标准事件流如何产出，作用域断言依赖它什么。
