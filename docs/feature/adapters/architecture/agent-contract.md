...
Agent setup 负责连接 Agent 自身并且每 attempt 只执行一次；环境预置属于 SandboxSpec，任务 fixture 属于 eval。setup 基础设施失败抛出为 errored，Agent 运行结果通过 Turn 表达。`setup` / `teardown` 遵循四层统一的成对语义——`teardown` 当且仅当本 attempt 走到过 agent setup 时点才执行,`setup` 抛错不豁免,状态经 Adapter 自己的闭包流动(见 [Runner · 环境预置](../../../runner.md#环境预置不进运行器但按顺序调它))。
