...
| `t.requireInputRequest(filter?)` | 要求最近一轮恰好有一个匹配的待输入请求，并返回它。`filter` 的字段全集是 `{ prompt?: string \| RegExp; action?: string; optionIds?: string[] }`：`prompt` 匹配请求向用户展示的提问文本，`action` 匹配请求的动作名（审批类请求即被审批的工具名，见 [Adapter · 返回待输入请求](../../adapters/library/sessions-and-hitl.md)），`optionIds` 要求请求恰好提供这组选项 |
