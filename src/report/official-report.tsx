...
// defaultReport 值(default-report.tsx),不是本组件;那侧的行为规范在
...
import { caseListData, overviewData, reasonFor, tableData } from "./compute.ts";
...
export interface VerdictBoardData {
  ...
  failing: {
    ...
    /**
     * 最新 attempt 的失败原因,按优先级取第一个在场的:`error` → `skipReason` →
     * 未通过的 gate 断言(原始声明顺序,detail 在场则 `"name: detail"`,多条用「, 」连接)。
     * 与 `MetricTable expand` 子行、`CaseList` 同一套材料(compute.ts 的 `reasonFor`)。
     */
    reason?: string;
...
function buildBoard(selection: Selection): VerdictBoardData {
  ...
  for (const snapshot of selection.snapshots) {
    ...
    for (const ev of snapshot.evals) {
      ...
      const reason = reasonFor(latest.result);
      failing.push({
        evalId: ev.id,
